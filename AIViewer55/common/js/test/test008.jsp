...
************************************************************************************
보고서 제목 : 보고서 (2013/1/31 18:57:50)
작성일 : 2013년1월31일 18시57분50초
작성자 :
************************************************************************************/
%>
...
<%!
private Logger logger = Logger.getLogger("test008.jsp");
class ReportHandler{
	//공통 변수 선언
	...
	private void getScriptFromCrossTable(boolean bIsPrint) throws Exception{
		
		long time=System.currentTimeMillis();
		System.out.println("CrossTable before freeMemory="+String.valueOf((Runtime.getRuntime().freeMemory())));
		
		CrossTable table = manager.createCrossTable();
		...
		for(int k=0; k<500; k++) {
			for(int i=0; i<128; i++)
			{
				...
				if(i%2==0) row.add("Mount Points");
				else row.add("Da1");
				row.add("mm");
				if(i<8) row.add("20110503");
				else if(i<16) row.add("201105015");
				else if(i<24) row.add("20110507");
				else if(i<32) row.add("20110505");
				else if(i<40) row.add("20110509");
				else if(i<48) row.add("20110511");
				else if(i<56) row.add("20110513");
				else if(i<64) row.add("20110501");
				else if(i<72) row.add("20110517");
				else if(i<80) row.add("20110519");
				else if(i<88) row.add("20110521");
				else if(i<96) row.add("20110523");
				else if(i<104) row.add("20110525");
				else if(i<112) row.add("20110527");
				else if(i<120) row.add("20110529");
				else row.add("20110531");
		...
		//sort 구현 부분
		Vector<Integer> sortindex=new Vector<Integer>();
		Vector<Boolean> sortorder=new Vector<Boolean>();
		Vector<String> sortcustom=new Vector<String>();
		...
		sortorder.add(true);
		sortorder.add(true);
		//sortcustom.add("Suwon<Suwon1<Samex<Samex1");
		//sortcustom.add("");
		
		//sortindex는 데이터셋의 필드 인덱스를 기준으로 함
		//sortorder는 오름차순(ascent) - true, 내림차순(descent) - fasle
		//sortcustom를 사용하지 않을 경우 null
		//사용하는 경우 벡터의 사이즈는 sortindex, sortorder 사이즈와 동일
		//customorder를 사용하지 않는 필드는 벡터에 빈문자열을 add
		//customorder는 <를 구분자로 하여 좌측 문자열이 sortorder와 상관없이 정렬우선순위를 가짐
		//입력된 내용 해석 - "Suwon<"  - Suwon은 모든 항목에 최우선 순위
		//				- "<Suwon"	- Suwon은 모든 항목에 가장 낮은 순위
		//				- "Suwon<Samex" - Suwon은 Samex에 보다 우선
		//						Suwon, So, Sa   ---> Suwon, Samex, So	오름차순
		//										---> So, Suwon, Samex   내림차순
		//table.sort(sortindex, sortorder, null);
		
		
		Vector<Integer> rowHeader=new Vector<Integer>();	//rowHeader field index
		Vector<Integer> colHeader=new Vector<Integer>();	//colHeader field index
		Vector<Integer> value=new Vector<Integer>();		//value field index - 현재 밸류 필드 인덱스는 하나만 존재
		...
		table.crossAnalysis(colHeader, rowHeader, value);	//피봇 함수
		
		table.summaryRows(1, null, "Total", false, new AIUtil.Sum(1));					//서머리 함수
		table.summaryCols(0, null, "Total", false, new AIUtil.Sum(0));					//서머리 함수
		
		//title header 
		Vector<String> titleHeader=new Vector<String>();
		titleHeader.add("Sectot");
		titleHeader.add("GBM");
		titleHeader.add("Product");
		titleHeader.add("Factory");
		titleHeader.add("Item");
		titleHeader.add("Unit");
		table.setTitleHeader(titleHeader);
		
		//cell name set
		table.setTitleHeaderCellName("sheet10_B1");
		table.setRowHeaderCellName("sheet10_B1");
		table.setColHeaderCellName("sheet10_B1");
		table.setDataCellName("sheet10_B1");
		
		//cell basic style set
		table.setTitleHeaderStyle("cell_style_0");
		table.setRowHeaderStyle("cell_style_0");
		table.setColHeaderStyle("cell_style_0");
		table.setDataStyle("cell_style_0");
		
		//width, height set
		table.setRowHeaderHeights(30);
		table.setRowHeaderWidths(70);
		table.setColHeaderHeights(30);
		table.setColHeaderWidths(150);
		...
		index.add(1);
		table.addRowHeaderProperty(index, text, "fillColor", "RGB[150,0,0]", true, true, true);
		index.clear();
		//text.clear();
		
		index.add(2);
		table.addRowHeaderProperty(index, text, "fillColor", "RGB[0,200,0]", true, true, true);
		index.clear();
		
		index.add(3);
		table.addRowHeaderProperty(index, text, "fillColor", "RGB[0,0,200]", true, true, true);
		index.clear();
		
		index.add(0);
		table.addColHeaderProperty(index, text, "fontBold", "n", true, true, true);
				
		index.clear();
		text.clear();
		
		Vector<Integer> colIndex=new Vector<Integer>();
		Vector<String> colText=new Vector<String>();
		index.add(3);
		index.add(4);
		text.add("Samex");
		text.add("Da1");
		colIndex.add(0);
		colText.add("20110517");
		table.addDataProperty(colIndex, colText, index, text, "fillColor", "RGB[0,255,0]");
		...
		link.put("type", "3");
		link.put("url", "http://wwww.ai.com");
		link.put("stream", "skdkel kkkkk");
		link.put("target", "new");

		colText.clear();
		colText.add(null);
		List<CrossTCell> datas=table.getDataCells(colIndex, colText, index, text);
		datas.get(0).setLinkInfo(link);
		datas.get(0).addStyleProperty("highLightPlusColorCell", "y");
		
		link.clear();
		link.put("url", "http://www.naver.com");
		datas.get(0).setLinkInfo(link);
		
		manager.scriptWrite(50, 100, table);
		
		System.out.println("CrossTable time = "+(System.currentTimeMillis()-time));
		System.out.println("CrossTable after freeMemory="+String.valueOf((Runtime.getRuntime().freeMemory())));
	}
	...
	private void writeScriptVoidBand100( ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos+80, nYCurPos+100, 80, 20, sheet10_B6, "sheet10_B6", "cell_style_0");
		objectWrite(CELL_OBJECT, nXCurPos+160, nYCurPos+100, 80, 20, sheet10_C6, "sheet10_C6", "cell_style_1");
		objectWrite(CELL_OBJECT, nXCurPos+240, nYCurPos+100, 80, 20, sheet10_D6, "sheet10_D6", "cell_style_1");
		objectWrite(CELL_OBJECT, nXCurPos+320, nYCurPos+100, 80, 20, sheet10_E6, "sheet10_E6", "cell_style_1");
	}
	...
	private void initConstVarientEx(){
		sheet10_B6="가가가가가가가가가가각";
		sheet10_C6="나나나나나나나나나나난";
		sheet10_D6="dkdkdkdkdkdkdkdkddk";
		sheet10_E6="fsdfk24242342424243";
	}
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|1|test008.style|보고서 (2013/1/31 18:57:50)", false);
