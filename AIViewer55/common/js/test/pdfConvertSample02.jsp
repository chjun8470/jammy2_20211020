...
<%!
private Logger logger = Logger.getLogger("pdfConvertSample02.jsp");
class ReportHandler{
	//공통 변수 선언
			...
			if(manager.open(reportMode, context.getRealPath(path)))
			{
				...
				for(int i =0; i < 10; i++){
					if(i == 0){
						...
						pdfStartPages.add(nMaxPage+1);
					}
					pdfFileNames.add("/log/AIPDF_" + sdf.format(System.currentTimeMillis())+ i + ".pdf");
	...
	private void executeSheet20( ) throws Exception{
		...
		//밴드 함수 호출
		getScriptFromb(true, true);    //Void 밴드 호출
		getScriptFromc(true, true);    //Void 밴드 호출
	...
	//----bIsPrint : 감추기 속성 설정 여부. true면 출력, false면 AI 스크립트를 생성하지 않음.
	//----nRepeatCnt : 부모 반복 밴드 수행 횟수와 연동 옵션 설정시 void 밴드가 수행할 반복 횟수 전달
	private void getScriptFromc(boolean bIsFirst, boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
		int nBandHeight=15;    //밴드 출력 높이
		int nRepeatNum=20;    //수행 횟수 지정
		int i;    //for문 카운트 변수
	...
	private void writeScripta( ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+54, nYCurPos+124, 98, 40, sheet10_B5, "sheet10_B5", "cell_style_3");
	...
	//----c 스크립트 문 출력 함수----
	private void writeScriptc( ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos, nYCurPos, 58, 15, "", "sheet20_A2", "cell_style_3");
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|2|pdfConvertSample02.style|XMD_0109_R01", false);
