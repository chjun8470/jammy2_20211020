...
************************************************************************************
보고서 제목 : 보고서 (2013/3/8 14:43:45)
작성일 : 2013년3월8일 14시43분45초
작성자 :
************************************************************************************/
%>
...
<%!
private Logger logger = Logger.getLogger("sample008.jsp");
class ReportHandler{
	//공통 변수 선언
	...
	private CSVSetHandler csv_db1=null;

	//-----쿼리 필드 변수 선언-----
	//UDS DataSet csv_db1 객체에서 필드 값을 받는 필드 변수
	private String csv_db1_num1;
	private String csv_db1_num2;
	private String csv_db1_num3;
	private String csv_db1_num4;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			//DataSet Close
			cleanup(csv_db1, null, null, null);
		}    //end try
	}
	...
	private void executeSheet10( ) throws Exception{
		...
		//밴드 함수 호출
		getScriptFromaaaa(true, true);    //Void 밴드 호출
	...
	private void getScriptFromaaaa(boolean bIsFirst, boolean bIsPrint) throws Exception{
		//지역 변수 선언 및 초기화
		int nBandHeight=140;    //밴드 출력 높이

		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//UDS DataSet csv_db1 객체의 생성 여부
			if(csv_db1==null){
				createUDSDataSetcsv_db1();
			}else{
				csv_db1.beforeFirst();
			}
			...
			//데이터 fetch
			if(csv_db1.next()){
				setFieldVariableFromcsv_db1();	//ResultSet csv_db1에서 필드 값 설정
			}else{
				setFieldInitFromcsv_db1();	//ResultSet csv_db1의 필드 값 초기화
			}
			...
			//데이터 변수 할당
			sheet10_B4=csv_db1_num1;
			sheet10_C4=csv_db1_num2;
			sheet10_D4=csv_db1_num3;
			sheet10_E4=csv_db1_num4;
		}    //end if(bIsFirst)
	...
	//-----스크립트 출력 함수-----
	//----aaaa 스크립트 문 출력 함수----
	private void writeScriptaaaa( ) throws Exception{
		//스크립트문 생성
		objectWrite(CELL_OBJECT, nXCurPos+80, nYCurPos, 80, 20, sheet10_B1, "sheet10_B1", "cell_style_0");
		objectWrite(CELL_OBJECT, nXCurPos+160, nYCurPos, 80, 20, sheet10_C1, "sheet10_C1", "cell_style_0");
		objectWrite(CELL_OBJECT, nXCurPos+80, nYCurPos+20, 80, 20, sheet10_B2, "sheet10_B2", "cell_style_1");
		propertyWrite(CELL_OBJECT, "sheet10_B2", "displayFormat", date_Param);
		objectWrite(CELL_OBJECT, nXCurPos+160, nYCurPos+20, 80, 20, sheet10_C2, "sheet10_C2", "cell_style_1");
		propertyWrite(CELL_OBJECT, "sheet10_C2", "displayFormat", date1_Param);
		objectWrite(CELL_OBJECT, nXCurPos+80, nYCurPos+40, 80, 20, sheet10_B3, "sheet10_B3", "cell_style_0");
		objectWrite(CELL_OBJECT, nXCurPos+160, nYCurPos+40, 80, 20, sheet10_C3, "sheet10_C3", "cell_style_0");
		objectWrite(CELL_OBJECT, nXCurPos+80, nYCurPos+60, 80, 20, sheet10_B4, "sheet10_B4", "cell_style_2");
		propertyWrite(CELL_OBJECT, "sheet10_B4", "displayFormat", num_Param);
		objectWrite(CELL_OBJECT, nXCurPos+160, nYCurPos+60, 80, 20, sheet10_C4, "sheet10_C4", "cell_style_2");
		propertyWrite(CELL_OBJECT, "sheet10_C4", "displayFormat", num1_Param);
		objectWrite(CELL_OBJECT, nXCurPos+240, nYCurPos+60, 80, 20, sheet10_D4, "sheet10_D4", "cell_style_3");
		objectWrite(CELL_OBJECT, nXCurPos+320, nYCurPos+60, 80, 20, sheet10_E4, "sheet10_E4", "cell_style_3");
	}
	...
	//UDS DataSet csv_db1 객체 생성 함수
	private void createUDSDataSetcsv_db1(){
		int csvSource;
		String csvUrl;
		String csvParams;
		String csvReqMethod;
		...
		csvSource=1;
		csvUrl="C:\\temp\\db110.csv";
		csvParams=null;
		csvReqMethod=null;
		csvMetaInfo="<num1><num2><num3><num4>";
		csvSeparator="COMMA";
		csvEncoding="8859_1";
		csvParsingLine=1;

		csv_db1=new CSVSetHandler(csvSource, csvUrl, csvParams, csvReqMethod);
		csv_db1.init(csvMetaInfo, csvSeparator, csvEncoding, csvParsingLine);
		csv_db1.makeDataSet();
	}

	//-----ResultSet 객체에서 필드 값 설정 함수-----
	//UDS DataSet csv_db1 객체에서 필드 값 설정 함수
	private void setFieldVariableFromcsv_db1() throws Exception{
		csv_db1_num1=GetB(csv_db1.getField(1));
		csv_db1_num2=GetB(csv_db1.getField(2));
		csv_db1_num3=GetB(csv_db1.getField(3));
		csv_db1_num4=GetB(csv_db1.getField(4));
	}

	//UDS DataSet csv_db1 객체의 필드 값 초기화 함수
	private void setFieldInitFromcsv_db1(){
		csv_db1_num1="";
		csv_db1_num2="";
		csv_db1_num3="";
		csv_db1_num4="";
	...
	private void setParam() {
		...
		date_Param=toKor(GetB(request.getParameter("date")));
		date_Param=aiReplace(date_Param, str, "\n");
		date1_Param=toKor(GetB(request.getParameter("date1")));
		date1_Param=aiReplace(date1_Param, str, "\n");
		num_Param=toKor(GetB(request.getParameter("num")));
		num_Param=aiReplace(num_Param, str, "\n");
		num1_Param=toKor(GetB(request.getParameter("num1")));
		num1_Param=aiReplace(num1_Param, str, "\n");
	}
	...
	private void getScriptHeader(String paper, String size, String value, String orient, String margin) throws Exception{
		...
		manager.scriptWrite("AIReport", "scriptVersion|docVersion|style|title", "32|1|sample008.style|보고서 (2013/3/8 14:43:45)", false);
