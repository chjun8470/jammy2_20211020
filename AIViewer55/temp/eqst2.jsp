...
class ReportHandler{
	...
	private void getScriptFromyyyyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 fetch
			if(bb.next()){
				setFieldVariableFrombb();	//ResultSet bb에서 필드 값 설정
			}else{
				setFieldInitFrombb();	//ResultSet bb의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet b 객체의 생성
		if(b==null){
			b = stat_b.executeQuery();
		}else{
			b.beforeFirst();
		}

		//--while Loop Start--##
		while(b.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromb();	//RecordSet b에서 필드 값 설정
			...
			//데이터 변수 할당
			sheet10_B21 = b_AREA;
			sheet10_H21 = b_ORG_CNT;
			sheet10_N21 = b_CNT;
			sheet10_R21 = b_EQUIP;
			sheet10_V21 = (long)b_SM;
	...
	private void writeScripttest( ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_V20"), nYCurPos+mapTop.get("sheet10_V20"), mapWidth.get("sheet10_V20"), mapHeight.get("sheet10_V20"), sheet10_V20, "sheet10_V20", mapStyle.get("sheet10_V20"));
	...
	private void writeScriptTbList(  ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_H21"), nYCurPos+mapTop.get("sheet10_H21"), mapWidth.get("sheet10_H21"), mapHeight.get("sheet10_H21"), String.valueOf(sheet10_H21), "sheet10_H21", mapStyle.get("sheet10_H21"));
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_V21"), nYCurPos+mapTop.get("sheet10_V21"), mapWidth.get("sheet10_V21"), mapHeight.get("sheet10_V21"), String.valueOf(sheet10_V21), "sheet10_V21", mapStyle.get("sheet10_V21"));
	}
	...
	private void getChartDataAI_CHART_OBJ32( ) throws Exception{
		...
		if(!b.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(b.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromb();   //RecordSet b에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + b_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + b_CNT + "\r\n";
		}   //while end
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_B20", 24, 372, 174, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet10_H20", 198, 372, 174, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_N20", 372, 372, 115, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_R20", 487, 372, 112, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_V20", 599, 372, 112, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet10_B21", 24, 0, 174, 20, "cell_style_7");
		setCellInfoMap_Internal("sheet10_H21", 198, 0, 174, 20, "cell_style_8");
		setCellInfoMap_Internal("sheet10_N21", 372, 0, 115, 20, "cell_style_8");
		setCellInfoMap_Internal("sheet10_R21", 487, 0, 112, 20, "cell_style_8");
		setCellInfoMap_Internal("sheet10_V21", 599, 0, 112, 20, "cell_style_9");
