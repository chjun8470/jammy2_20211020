...
public void jspInit() {
	...
	sql_jippppp.append("	COUNT(0) AS CNT, ");
	sql_jippppp.append("	A.REGIST_INFO, ");
	sql_jippppp.append("	A.TRANSFER_PERSON, ");
	sql_jippppp.append("	ROUND((SUM(A.FLAT_SUM_SKILL_FEE) / 10000)) AS FLAT_SUM, ");
	sql_jippppp.append("	ROUND((SUM(A.ORDINARY_SKILL_FEE) / 10000)) AS ORDINARY_SUM, ");
	sql_jippppp.append("	A.STATE, ");
	sql_jippppp.append("	A.ORG_GRP_CD, ");
	sql_jippppp.append("	B.ORG_GRP_NM ");
	sql_jippppp.append("FROM TB_JIP_USE_STATUS AS A ");
	sql_jippppp.append("LEFT JOIN TB_ORG_GRP AS B ");
	sql_jippppp.append("ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_jippppp.append("WHERE A.STATE = '2' ");
	sql_jippppp.append("AND B.ORG_GRP_CD = '^orgcd^' ");
	sql_jippppp.append("GROUP BY A.TRANSFER_PERSON ");
	sql_jippppp.append("ORDER BY CNT DESC ");
	...
	sql_TTT1.append("AND B.ORG_GRP_CD = '^orgcd^' ");
	sql_TTT1.append("GROUP BY A.TRANSFER_PERSON ");
	...
	sql_sss1.append("AND B.ORG_GRP_CD = '^orgcd^' ");
	sql_sss1.append("GROUP BY A.TRANSFER_PERSON ");
...
class ReportHandler{
	...
	private long jippppp_FLAT_SUM;
	private double jippppp_ORDINARY_SUM;
	private String jippppp_STATE;
	private String jippppp_ORG_GRP_CD;
	private String jippppp_ORG_GRP_NM;
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//DataSet 객체 생성
		//ResultSet jippppp 객체의 생성
		if(jippppp==null){
			jippppp = stat_jippppp.executeQuery();
		}else{
			jippppp.beforeFirst();
		}

		//--while Loop Start--##
		while(jippppp.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromjippppp();	//RecordSet jippppp에서 필드 값 설정

			if(bIsFirst){   //밴드 호출 후 루프 내에서 한번 만 실행되는 코드
			}   //end if(bIsFirst)

			//데이터 변수 할당
			sheet10_B23 = jippppp_REGIST_INFO;
			sheet10_I23 = jippppp_TRANSFER_PERSON;
			sheet10_O23 = jippppp_FLAT_SUM;
			sheet10_U23 = (long)jippppp_ORDINARY_SUM;
	...
	private void writeScriptview( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos, nYCurPos+31, 726, 368, AI_CHART_OBJ43, "AI_CHART_OBJ43", "draw_style_0");
	}
	...
	private void writeScriptviewList( ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_U22"), nYCurPos+mapTop.get("sheet10_U22"), mapWidth.get("sheet10_U22"), mapHeight.get("sheet10_U22"), sheet10_U22, "sheet10_U22", mapStyle.get("sheet10_U22"));
	}
	...
	private void writeScriptTbList(  ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_U23"), nYCurPos+mapTop.get("sheet10_U23"), mapWidth.get("sheet10_U23"), mapHeight.get("sheet10_U23"), String.valueOf(sheet10_U23), "sheet10_U23", mapStyle.get("sheet10_U23"));
	}
	...
	private void setFieldVariableFromjippppp() throws Exception{
		jippppp_CNT = jippppp.getLong(1);
		jippppp_REGIST_INFO = GetB(jippppp.getString(2));
		jippppp_TRANSFER_PERSON = GetB(jippppp.getString(3));
		jippppp_FLAT_SUM = jippppp.getLong(4);
		jippppp_ORDINARY_SUM = jippppp.getDouble(5);
		jippppp_STATE = GetB(jippppp.getString(6));
		jippppp_ORG_GRP_CD = GetB(jippppp.getString(7));
		jippppp_ORG_GRP_NM = GetB(jippppp.getString(8));
	}
	...
	private List<Object> getAIRowDataFromjippppp() throws Exception{
		...
		row.add(jippppp.getLong(1));
		row.add(jippppp.getObject(2));
		row.add(jippppp.getObject(3));
		row.add(jippppp.getLong(4));
		row.add(jippppp.getDouble(5));
		row.add(jippppp.getObject(6));
		row.add(jippppp.getObject(7));
	...
	//----AI_CHART_OBJ43 챠트 데이터 함수----
	private void getChartDataAI_CHART_OBJ43( ) throws Exception{
		//DataSet 객체 생성
		//ResultSet jippppp 객체의 생성
		if(jippppp==null){
			jippppp = stat_jippppp.executeQuery();
		...
		if(!jippppp.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}

		//계열(Series) 필드 설정
		AI_CHART_OBJ43="\t";
		AI_CHART_OBJ43=AI_CHART_OBJ43 + "CNT\r\n";

		while(jippppp.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromjippppp();   //RecordSet jippppp에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ43=AI_CHART_OBJ43 + jippppp_TRANSFER_PERSON + "\t";

			//데이터 설정
			AI_CHART_OBJ43=AI_CHART_OBJ43 + jippppp_CNT + "\r\n";
		}   //while end
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_O22", 401, 0, 170, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_U22", 571, 0, 140, 20, "cell_style_6");
		...
		setCellInfoMap_Internal("sheet10_O23", 401, 0, 170, 20, "cell_style_9");
		setCellInfoMap_Internal("sheet10_U23", 571, 0, 140, 20, "cell_style_10");
