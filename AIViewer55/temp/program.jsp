...
public void jspInit() {
	...
	sql_a5.append("				WHERE STATE = '2' ");
	sql_a5.append("			) AS A ");
	sql_a5.append("			LEFT JOIN TB_ORG_GRP AS B ");
	sql_a5.append("			ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_a5.append("			WHERE 1 = 1 ");
	sql_a5.append("AND A.IDX = '5' ");
	sql_a5.append("GROUP BY A.ORG_NM ");
	sql_a5.append("ORDER BY CNT DESC ");
	...
	//sql_a5a5
	StringBuilder sql_a5a5 = new StringBuilder();
	sql_a5a5.setLength(0);
	sql_a5a5.append("SELECT ");
	sql_a5a5.append("				A.IDX, ");
	sql_a5a5.append("				A.ORG_NM AS AREA, ");
	sql_a5a5.append("				A.ORG_GRP_CD, ");
	sql_a5a5.append("				COUNT(0) AS CNT ");
	sql_a5a5.append("			FROM( ");
	sql_a5a5.append("				SELECT ");
	sql_a5a5.append("					'1' AS IDX , ");
	sql_a5a5.append("					STATE AS STATE, ");
	sql_a5a5.append("					INTRL_APLC_NO AS APLC_NO, ");
	sql_a5a5.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a5a5.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a5a5.append("				FROM TB_SEARCH_BRAND ");
	sql_a5a5.append("				WHERE STATE = '2' ");
	sql_a5a5.append("				UNION ALL ");
	sql_a5a5.append("				SELECT ");
	sql_a5a5.append("					'2' AS IDX , ");
	sql_a5a5.append("					STATE AS STATE, ");
	sql_a5a5.append("					APLC_NO AS APLC_NO, ");
	sql_a5a5.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a5a5.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a5a5.append("				FROM TB_SEARCH_DESIGN ");
	sql_a5a5.append("				WHERE STATE = '2' ");
	sql_a5a5.append("				UNION ALL ");
	sql_a5a5.append("				SELECT ");
	sql_a5a5.append("					'3' AS IDX , ");
	sql_a5a5.append("					STATE AS STATE, ");
	sql_a5a5.append("					KNOW_HOW_NO AS APLC_NO, ");
	sql_a5a5.append("					ORG_NM AS ORG_NM, ");
	sql_a5a5.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a5a5.append("				FROM TB_SEARCH_KNOW_HOW ");
	sql_a5a5.append("				WHERE STATE = '2' ");
	sql_a5a5.append("				UNION ALL ");
	sql_a5a5.append("				SELECT ");
	sql_a5a5.append("					'4' AS IDX , ");
	sql_a5a5.append("					STATE AS STATE, ");
	sql_a5a5.append("					APLC_NO AS APLC_NO, ");
	sql_a5a5.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a5a5.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a5a5.append("				FROM TB_SEARCH_PATENT_UTILITY_MODEL ");
	sql_a5a5.append("				WHERE STATE = '2' ");
	sql_a5a5.append("				UNION ALL ");
	sql_a5a5.append("				SELECT ");
	sql_a5a5.append("					'5' AS IDX , ");
	sql_a5a5.append("					STATE AS STATE, ");
	sql_a5a5.append("					REG_NO AS APLC_NO, ");
	sql_a5a5.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a5a5.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a5a5.append("				FROM TB_SEARCH_PROGRAM ");
	sql_a5a5.append("				WHERE STATE = '2' ");
	sql_a5a5.append("			) AS A ");
	sql_a5a5.append("			LEFT JOIN TB_ORG_GRP AS B ");
	sql_a5a5.append("			ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_a5a5.append("			WHERE 1 = 1 ");
	sql_a5a5.append("AND A.IDX = '5' ");
	sql_a5a5.append("GROUP BY A.ORG_NM ");
	sql_a5a5.append("ORDER BY CNT DESC ");
	sql_a5a5.append("LIMIT 10 ");
	initializer.prepareQuery("sql_a5a5", sql_a5a5.toString());
}
...
class ReportHandler{
	...
	private java.sql.PreparedStatement stat_a5a5;		//ResultSet a5a5 객체를 생성하기 위한 PreparedStatement 객체
	private ResultSet a5a5;		//ResultSet a5a5 객체
	...
	private StringBuilder sql_a5a5 = new StringBuilder();		//ResultSet a5a5 객체를 생성하기 위한 쿼리문 저장
	...
	//ResultSet a5a5 객체에서 필드 값을 받는 쿼리 필드 변수
	private String a5a5_IDX;
	private String a5a5_AREA;
	private String a5a5_ORG_GRP_CD;
	private long a5a5_CNT;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			//DataSet Close
			cleanup(a5a5, stat_a5a5, null, null);
	...
	private void getScriptFromyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet a5a5 객체의 생성
			if(a5a5==null){
				a5a5 = stat_a5a5.executeQuery();
			}else{
				a5a5.beforeFirst();
			}
			...
			//데이터 fetch
			if(a5a5.next()){
				setFieldVariableFroma5a5();	//ResultSet a5a5에서 필드 값 설정
			}else{
				setFieldInitFroma5a5();	//ResultSet a5a5의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet a5 객체의 생성
		if(a5==null){
			a5 = stat_a5.executeQuery();
		}else{
			a5.beforeFirst();
		}

		//--while Loop Start--##
		while(a5.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFroma5();	//RecordSet a5에서 필드 값 설정
	...
	private void writeScriptview( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos, nYCurPos+32, 726, 362, AI_CHART_OBJ41, "AI_CHART_OBJ41", "draw_style_0");
	}
	...
	private void initSQLConst() throws Exception {
		...
		stat_a5a5 = initializer.configPreparedStatement("sql_a5a5", mapParam, conn_jnsp, null);
	}
	...
	private void setFieldVariableFroma5() throws Exception{
		...
		a5_ORG_GRP_CD = GetB(a5.getString(3));
		a5_CNT = a5.getLong(4);
	}
	...
	private List<Object> getAIRowDataFroma5() throws Exception{
		...
		row.add(a5.getObject(3));
		row.add(a5.getLong(4));
	...
	//ResultSet a5a5 객체에서 필드 값 설정 함수
	private void setFieldVariableFroma5a5() throws Exception{
		a5a5_IDX = GetB(a5a5.getString(1));
		a5a5_AREA = GetB(a5a5.getString(2));
		a5a5_ORG_GRP_CD = GetB(a5a5.getString(3));
		a5a5_CNT = a5a5.getLong(4);
	}

	//ResultSet a5a5 객체의 필드 값 초기화 함수
	private void setFieldInitFroma5a5(){
		a5a5_IDX = "";
		a5a5_AREA = "";
		a5a5_ORG_GRP_CD = "";
		a5a5_CNT = 0L;
	...
	//ResultSet a5a5 객체에서 RowData 획득함수
	private List<Object> getAIRowDataFroma5a5() throws Exception{
		List<Object> row = new ArrayList<Object>();

		row.add(a5a5.getObject(1));
		row.add(a5a5.getObject(2));
		row.add(a5a5.getObject(3));
		row.add(a5a5.getLong(4));
	...
	private void getChartDataAI_CHART_OBJ41( ) throws Exception{
		...
		if(!a5.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(a5.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFroma5();   //RecordSet a5에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a5_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a5_CNT + "\r\n";
		}   //while end
	}
