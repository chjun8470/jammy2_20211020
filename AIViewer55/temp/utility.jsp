...
public void jspInit() {
	...
	sql_a4.append("				WHERE STATE = '2' ");
	sql_a4.append("			) AS A ");
	sql_a4.append("			LEFT JOIN TB_ORG_GRP AS B ");
	sql_a4.append("			ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_a4.append("			WHERE 1 = 1 ");
	sql_a4.append("AND A.IDX = '4' ");
	sql_a4.append("GROUP BY A.ORG_NM ");
	sql_a4.append("ORDER BY CNT DESC ");
	sql_a4.append("LIMIT 10 ");
	initializer.prepareQuery("sql_a4", sql_a4.toString());

	//sql_a4a4
	StringBuilder sql_a4a4 = new StringBuilder();
	sql_a4a4.setLength(0);
	sql_a4a4.append("SELECT ");
	sql_a4a4.append("				A.IDX, ");
	sql_a4a4.append("				A.ORG_NM AS AREA, ");
	sql_a4a4.append("				A.ORG_GRP_CD, ");
	sql_a4a4.append("				COUNT(0) AS CNT ");
	sql_a4a4.append("			FROM( ");
	sql_a4a4.append("				SELECT ");
	sql_a4a4.append("					'1' AS IDX , ");
	sql_a4a4.append("					STATE AS STATE, ");
	sql_a4a4.append("					INTRL_APLC_NO AS APLC_NO, ");
	sql_a4a4.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a4a4.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a4a4.append("				FROM TB_SEARCH_BRAND ");
	sql_a4a4.append("				WHERE STATE = '2' ");
	sql_a4a4.append("				UNION ALL ");
	sql_a4a4.append("				SELECT ");
	sql_a4a4.append("					'2' AS IDX , ");
	sql_a4a4.append("					STATE AS STATE, ");
	sql_a4a4.append("					APLC_NO AS APLC_NO, ");
	sql_a4a4.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a4a4.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a4a4.append("				FROM TB_SEARCH_DESIGN ");
	sql_a4a4.append("				WHERE STATE = '2' ");
	sql_a4a4.append("				UNION ALL ");
	sql_a4a4.append("				SELECT ");
	sql_a4a4.append("					'3' AS IDX , ");
	sql_a4a4.append("					STATE AS STATE, ");
	sql_a4a4.append("					KNOW_HOW_NO AS APLC_NO, ");
	sql_a4a4.append("					ORG_NM AS ORG_NM, ");
	sql_a4a4.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a4a4.append("				FROM TB_SEARCH_KNOW_HOW ");
	sql_a4a4.append("				WHERE STATE = '2' ");
	sql_a4a4.append("				UNION ALL ");
	sql_a4a4.append("				SELECT ");
	sql_a4a4.append("					'4' AS IDX , ");
	sql_a4a4.append("					STATE AS STATE, ");
	sql_a4a4.append("					APLC_NO AS APLC_NO, ");
	sql_a4a4.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a4a4.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a4a4.append("				FROM TB_SEARCH_PATENT_UTILITY_MODEL ");
	sql_a4a4.append("				WHERE STATE = '2' ");
	sql_a4a4.append("				UNION ALL ");
	sql_a4a4.append("				SELECT ");
	sql_a4a4.append("					'5' AS IDX , ");
	sql_a4a4.append("					STATE AS STATE, ");
	sql_a4a4.append("					REG_NO AS APLC_NO, ");
	sql_a4a4.append("					OWN_ORG_NM AS ORG_NM, ");
	sql_a4a4.append("					ORG_GRP_CD AS ORG_GRP_CD ");
	sql_a4a4.append("				FROM TB_SEARCH_PROGRAM ");
	sql_a4a4.append("				WHERE STATE = '2' ");
	sql_a4a4.append("			) AS A ");
	sql_a4a4.append("			LEFT JOIN TB_ORG_GRP AS B ");
	sql_a4a4.append("			ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_a4a4.append("			WHERE 1 = 1 ");
	sql_a4a4.append("AND A.IDX = '4' ");
	sql_a4a4.append("GROUP BY A.ORG_NM ");
	sql_a4a4.append("ORDER BY CNT DESC ");
	sql_a4a4.append("LIMIT 10 ");
	initializer.prepareQuery("sql_a4a4", sql_a4a4.toString());
}
...
class ReportHandler{
	...
	private java.sql.PreparedStatement stat_a4a4;		//ResultSet a4a4 객체를 생성하기 위한 PreparedStatement 객체
	private ResultSet a4a4;		//ResultSet a4a4 객체
	...
	private StringBuilder sql_a4a4 = new StringBuilder();		//ResultSet a4a4 객체를 생성하기 위한 쿼리문 저장
	...
	private String a4_ORG_GRP_CD;
	private long a4_CNT;

	//ResultSet a4a4 객체에서 필드 값을 받는 쿼리 필드 변수
	private String a4a4_IDX;
	private String a4a4_AREA;
	private String a4a4_ORG_GRP_CD;
	private long a4a4_CNT;
	...
	void service(JspWriter out) throws Exception{
		try{    //try 구문
			...
			//DataSet Close
			cleanup(a4a4, stat_a4a4, null, null);
	...
	private void getScriptFromyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet a4a4 객체의 생성
			if(a4a4==null){
				a4a4 = stat_a4a4.executeQuery();
			}else{
				a4a4.beforeFirst();
			}
			...
			//데이터 fetch
			if(a4a4.next()){
				setFieldVariableFroma4a4();	//ResultSet a4a4에서 필드 값 설정
			}else{
				setFieldInitFroma4a4();	//ResultSet a4a4의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet a4 객체의 생성
		if(a4==null){
			a4 = stat_a4.executeQuery();
		}else{
			a4.beforeFirst();
		}

		//--while Loop Start--##
		while(a4.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFroma4();	//RecordSet a4에서 필드 값 설정
	...
	private void initSQLConst() throws Exception {
		...
		stat_a4a4 = initializer.configPreparedStatement("sql_a4a4", mapParam, conn_jnsp, null);
	}
	...
	private void setFieldVariableFroma4() throws Exception{
		...
		a4_ORG_GRP_CD = GetB(a4.getString(3));
		a4_CNT = a4.getLong(4);
	}
	...
	private List<Object> getAIRowDataFroma4() throws Exception{
		...
		row.add(a4.getObject(3));
		row.add(a4.getLong(4));

		return row;
	}

	//ResultSet a4a4 객체에서 필드 값 설정 함수
	private void setFieldVariableFroma4a4() throws Exception{
		a4a4_IDX = GetB(a4a4.getString(1));
		a4a4_AREA = GetB(a4a4.getString(2));
		a4a4_ORG_GRP_CD = GetB(a4a4.getString(3));
		a4a4_CNT = a4a4.getLong(4);
	}

	//ResultSet a4a4 객체의 필드 값 초기화 함수
	private void setFieldInitFroma4a4(){
		a4a4_IDX = "";
		a4a4_AREA = "";
		a4a4_ORG_GRP_CD = "";
		a4a4_CNT = 0L;
	...
	//ResultSet a4a4 객체에서 RowData 획득함수
	private List<Object> getAIRowDataFroma4a4() throws Exception{
		List<Object> row = new ArrayList<Object>();

		row.add(a4a4.getObject(1));
		row.add(a4a4.getObject(2));
		row.add(a4a4.getObject(3));
		row.add(a4a4.getLong(4));
	...
	private void getChartDataAI_CHART_OBJ41( ) throws Exception{
		...
		if(!a4.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(a4.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFroma4();   //RecordSet a4에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a4_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a4_CNT + "\r\n";
		}   //while end
	}
