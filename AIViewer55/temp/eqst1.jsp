...
class ReportHandler{
	...
	private void getScriptFromyyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 fetch
			if(aa.next()){
				setFieldVariableFromaa();	//ResultSet aa에서 필드 값 설정
			}else{
				setFieldInitFromaa();	//ResultSet aa의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//--while Loop Start--##
		while(a.next() && b.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFroma();	//RecordSet a에서 필드 값 설정
	...
	private void getChartDataAI_CHART_OBJ32( ) throws Exception{
		...
		if(!a.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(a.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFroma();   //RecordSet a에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + a_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + a_CNT + "\r\n";
		}   //while end
	}
