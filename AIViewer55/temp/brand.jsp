...
public void jspInit() {
	...
	sql_a1.append("ORDER BY CNT DESC ");
	...
	sql_a1a1.append("ORDER BY CNT DESC ");
...
class ReportHandler{
	...
	private void getScriptFromyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet a1a1 객체의 생성
			if(a1a1==null){
				a1a1 = stat_a1a1.executeQuery();
			}else{
				a1a1.beforeFirst();
			}
			...
			//데이터 fetch
			if(a1a1.next()){
				setFieldVariableFroma1a1();	//ResultSet a1a1에서 필드 값 설정
			}else{
				setFieldInitFroma1a1();	//ResultSet a1a1의 필드 값 초기화
			}
	...
	private void getChartDataAI_CHART_OBJ41( ) throws Exception{
		...
		if(!a1.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(a1.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFroma1();   //RecordSet a1에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a1_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a1_CNT + "\r\n";
		}   //while end
	}
