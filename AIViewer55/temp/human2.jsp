...
class ReportHandler{
	...
	private void getScriptFromyyyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 fetch
			if(dd.next()){
				setFieldVariableFromdd();	//ResultSet dd에서 필드 값 설정
			}else{
				setFieldInitFromdd();	//ResultSet dd의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet d 객체의 생성
		if(d==null){
			d = stat_d.executeQuery();
		}else{
			d.beforeFirst();
		}

		//--while Loop Start--##
		while(d.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromd();	//RecordSet d에서 필드 값 설정
	...
	private void writeScriptTbList(  ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_K21"), nYCurPos+mapTop.get("sheet10_K21"), mapWidth.get("sheet10_K21"), mapHeight.get("sheet10_K21"), String.valueOf(sheet10_K21), "sheet10_K21", mapStyle.get("sheet10_K21"));
	...
	private void getChartDataAI_CHART_OBJ32( ) throws Exception{
		...
		if(!d.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(d.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromd();   //RecordSet d에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + d_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + d_CNT + "\r\n";
		}   //while end
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_B20", 24, 372, 261, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet10_K20", 285, 372, 202, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet10_R20", 487, 372, 224, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet10_B21", 24, 0, 261, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_K21", 285, 0, 202, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet10_R21", 487, 0, 224, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet10_B22", 24, 0, 29, 20, "cell_style_7");
	}
