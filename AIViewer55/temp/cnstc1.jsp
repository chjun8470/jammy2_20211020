...
public void jspInit() {
	...
	sql_a.setLength(0);
	sql_a.append("SELECT ");
	sql_a.append("	A.RND_MAIN_ORG AS AXIS , ");
	sql_a.append("	B.ORG_GRP_NM, ");
	sql_a.append("	C.DEPT_NM, ");
	sql_a.append("	SUM(A.TAKE_PRC_FOREIGN) AS SM, ");
	sql_a.append("	COUNT(*) AS CNT ");
	sql_a.append("FROM ");
	sql_a.append("	TB_EQU_INFO AS A ");
	sql_a.append("LEFT JOIN TB_ORG_GRP AS B ");
	sql_a.append("	ON A.RND_MAIN_ORG = B.ORG_GRP_NM ");
	sql_a.append("LEFT JOIN TB_USER_COPY AS C ");
	sql_a.append("	ON A.USER_IDX = C.USER_IDX ");
	sql_a.append("WHERE 1 = 1 ");
	sql_a.append("group by A.RND_MAIN_ORG ");
	...
	sql_b.setLength(0);
	sql_b.append("SELECT ");
	sql_b.append("	A.KOR_NM, ");
	sql_b.append("	SUM(A.TAKE_PRC_FOREIGN) AS SM, ");
	sql_b.append("	SUBSTRING(A.LOCATION,4,3) AS AXIS, ");
	sql_b.append("	COUNT(*) AS CNT ");
	sql_b.append("FROM ");
	sql_b.append("	TB_EQU_INFO AS A ");
	sql_b.append("INNER JOIN TB_USER_COPY AS B ");
	sql_b.append("	ON A.USER_IDX = B.USER_IDX ");
	sql_b.append("INNER JOIN TB_ORG_GRP AS C ");
	sql_b.append("	ON B.USER_IDX = C.USER_IDX ");
	sql_b.append("WHERE 1 = 1 ");
	sql_b.append("	AND C.AREA_CD = 1 ");
	sql_b.append("GROUP BY SUBSTRING(A.LOCATION,4,3) ");
	sql_b.append("ORDER BY SUBSTRING(A.LOCATION,4,3) ASC ");
...
class ReportHandler{
	...
	private String b_KOR_NM;
	private double b_SM;
	private String b_AXIS;
	private long b_CNT;
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet c 객체의 생성
		if(c==null){
			c = stat_c.executeQuery();
		}else{
			c.beforeFirst();
		}

		//--while Loop Start--##
		while(c.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromc();	//RecordSet c에서 필드 값 설정
	...
	private void writeScripttest( ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_K19"), nYCurPos+mapTop.get("sheet10_K19"), mapWidth.get("sheet10_K19"), mapHeight.get("sheet10_K19"), sheet10_K19, "sheet10_K19", mapStyle.get("sheet10_K19"));
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_S19"), nYCurPos+mapTop.get("sheet10_S19"), mapWidth.get("sheet10_S19"), mapHeight.get("sheet10_S19"), sheet10_S19, "sheet10_S19", mapStyle.get("sheet10_S19"));
		objectWrite(DRAW_OBJECT, nXCurPos+2, nYCurPos+31, 724, 327, AI_CHART_OBJ31, "AI_CHART_OBJ31", "draw_style_0");
	}
	...
	private void writeScriptTbList(  ) throws Exception{
		...
		objectWrite(CELL_OBJECT, nXCurPos+mapLeft.get("sheet10_S20"), nYCurPos+mapTop.get("sheet10_S20"), mapWidth.get("sheet10_S20"), mapHeight.get("sheet10_S20"), String.valueOf(sheet10_S20), "sheet10_S20", mapStyle.get("sheet10_S20"));
	}
	...
	private void setFieldVariableFroma() throws Exception{
		a_AXIS = GetB(a.getString(1));
		a_ORG_GRP_NM = GetB(a.getString(2));
		a_DEPT_NM = GetB(a.getString(3));
		a_SM = a.getDouble(4);
		a_CNT = a.getLong(5);
	}
	...
	private void setFieldVariableFromb() throws Exception{
		b_KOR_NM = GetB(b.getString(1));
		b_SM = b.getDouble(2);
		b_AXIS = GetB(b.getString(3));
		b_CNT = b.getLong(4);
	}
	...
	private List<Object> getAIRowDataFromb() throws Exception{
		...
		row.add(b.getDouble(2));
		row.add(b.getObject(3));
		row.add(b.getLong(4));
	...
	private void getChartDataAI_CHART_OBJ31( ) throws Exception{
		...
		if(!c.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}

		//계열(Series) 필드 설정
		AI_CHART_OBJ31="\t";
		AI_CHART_OBJ31=AI_CHART_OBJ31 + "CNT\r\n";

		while(c.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromc();   //RecordSet c에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ31=AI_CHART_OBJ31 + c_AXIS + "\t";

			//데이터 설정
			AI_CHART_OBJ31=AI_CHART_OBJ31 + c_CNT + "\r\n";
		}   //while end
	}
	...
	public void setCellInfoMap() throws Exception {
		...
		setCellInfoMap_Internal("sheet10_B19", 24, 372, 261, 20, "cell_style_3");
		setCellInfoMap_Internal("sheet10_K19", 285, 372, 230, 20, "cell_style_4");
		setCellInfoMap_Internal("sheet10_S19", 515, 372, 196, 20, "cell_style_5");
		setCellInfoMap_Internal("sheet10_B20", 24, 0, 261, 20, "cell_style_6");
		setCellInfoMap_Internal("sheet10_K20", 285, 0, 230, 20, "cell_style_7");
		setCellInfoMap_Internal("sheet10_S20", 515, 0, 196, 20, "cell_style_8");
	}
