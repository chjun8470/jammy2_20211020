...
public void jspInit() {
	...
	sql_jip2.append("ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_jip2.append("WHERE 1 = 1 ");
	sql_jip2.append("AND B.ORG_GRP_CD = '^orgcd^' ");
	...
	sql_ttt1.append("	A.IDX, ");
	sql_ttt1.append("	A.ORG_NM AS AREA, ");
	sql_ttt1.append("	A.DEPT_NM , ");
	sql_ttt1.append("	A.ORG_GRP_CD , ");
	sql_ttt1.append("	B.ORG_GRP_NM, ");
	sql_ttt1.append("	COUNT(0) AS CNT ");
	sql_ttt1.append("FROM( ");
	sql_ttt1.append("	SELECT ");
	sql_ttt1.append("		'상표' AS IDX , ");
	sql_ttt1.append("		INTRL_APLC_NO AS APLC_NO, ");
	sql_ttt1.append("		OWN_ORG_NM AS ORG_NM, ");
	sql_ttt1.append("		ORG_GRP_CD AS ORG_GRP_CD, ");
	sql_ttt1.append("		OWN_DEPT_NM AS DEPT_NM ");
	sql_ttt1.append("	FROM TB_SEARCH_BRAND ");
	sql_ttt1.append("	UNION ALL ");
	sql_ttt1.append("	SELECT ");
	sql_ttt1.append("		'디자인' AS IDX , ");
	sql_ttt1.append("		APLC_NO AS APLC_NO, ");
	sql_ttt1.append("		OWN_ORG_NM AS ORG_NM, ");
	sql_ttt1.append("		ORG_GRP_CD AS ORG_GRP_CD, ");
	sql_ttt1.append("		OWN_DEPT_NM AS DEPT_NM ");
	sql_ttt1.append("	FROM TB_SEARCH_DESIGN ");
	sql_ttt1.append("	UNION ALL ");
	sql_ttt1.append("	SELECT ");
	sql_ttt1.append("		'노하우' AS IDX , ");
	sql_ttt1.append("		KNOW_HOW_NO AS APLC_NO, ");
	sql_ttt1.append("		ORG_NM AS ORG_NM, ");
	sql_ttt1.append("		ORG_GRP_CD AS ORG_GRP_CD, ");
	sql_ttt1.append("		DEPT_NM AS DEPT_NM ");
	sql_ttt1.append("	FROM TB_SEARCH_KNOW_HOW ");
	sql_ttt1.append("	UNION ALL ");
	sql_ttt1.append("	SELECT ");
	sql_ttt1.append("		'특허실용' AS IDX , ");
	sql_ttt1.append("		APLC_NO AS APLC_NO, ");
	sql_ttt1.append("		OWN_ORG_NM AS ORG_NM, ");
	sql_ttt1.append("		ORG_GRP_CD AS ORG_GRP_CD, ");
	sql_ttt1.append("		OWN_DEPT_NM AS DEPT_NM ");
	sql_ttt1.append("	FROM TB_SEARCH_PATENT_UTILITY_MODEL ");
	sql_ttt1.append("	UNION ALL ");
	sql_ttt1.append("	SELECT ");
	sql_ttt1.append("		'프로그램' AS IDX , ");
	sql_ttt1.append("		REG_NO AS APLC_NO, ");
	sql_ttt1.append("		OWN_ORG_NM AS ORG_NM, ");
	sql_ttt1.append("		ORG_GRP_CD AS ORG_GRP_CD, ");
	sql_ttt1.append("		OWN_DEPT_NM AS DEPT_NM ");
	sql_ttt1.append("	FROM TB_SEARCH_PROGRAM ");
	sql_ttt1.append(") AS A ");
	sql_ttt1.append("INNER JOIN TB_ORG_GRP AS B ");
	sql_ttt1.append("ON A.ORG_GRP_CD = B.ORG_GRP_CD ");
	sql_ttt1.append("WHERE 1 = 1 ");
	sql_ttt1.append("AND B.ORG_GRP_CD = '^orgcd^' ");
	sql_ttt1.append("GROUP BY A.IDX ");
...
class ReportHandler{
	...
	private String jip2_ORG_GRP_CD;
	private String jip2_ORG_GRP_NM;
	private long jip2_CNT;

	//ResultSet ttt1 객체에서 필드 값을 받는 쿼리 필드 변수
	private String ttt1_IDX;
	private String ttt1_AREA;
	private String ttt1_DEPT_NM;
	...
	private void getScriptFromyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet ttt1 객체의 생성
			if(ttt1==null){
				ttt1 = stat_ttt1.executeQuery();
			}else{
				ttt1.beforeFirst();
			}
			...
			//데이터 fetch
			if(ttt1.next()){
				setFieldVariableFromttt1();	//ResultSet ttt1에서 필드 값 설정
			}else{
				setFieldInitFromttt1();	//ResultSet ttt1의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//DataSet 객체 생성
		//ResultSet jip2 객체의 생성
		if(jip2==null){
			jip2 = stat_jip2.executeQuery();
		}else{
			jip2.beforeFirst();
		}

		//--while Loop Start--##
		while(jip2.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromjip2();	//RecordSet jip2에서 필드 값 설정

			if(bIsFirst){   //밴드 호출 후 루프 내에서 한번 만 실행되는 코드
			}   //end if(bIsFirst)

			//데이터 변수 할당
			sheet10_B23 = jip2_IDX;
			sheet10_N23 = jip2_CNT;
	...
	private void writeScriptview( ) throws Exception{
		...
		objectWrite(DRAW_OBJECT, nXCurPos, nYCurPos+32, 724, 368, AI_CHART_OBJ43, "AI_CHART_OBJ43", "draw_style_0");
	}
	...
	private void setFieldVariableFromjip2() throws Exception{
		...
		jip2_AREA = GetB(jip2.getString(2));
		jip2_DEPT_NM = GetB(jip2.getString(3));
		jip2_ORG_GRP_CD = GetB(jip2.getString(4));
	...
	private void setFieldVariableFromttt1() throws Exception{
		ttt1_IDX = GetB(ttt1.getString(1));
		ttt1_AREA = GetB(ttt1.getString(2));
		ttt1_DEPT_NM = GetB(ttt1.getString(3));
		ttt1_ORG_GRP_CD = GetB(ttt1.getString(4));
		ttt1_ORG_GRP_NM = GetB(ttt1.getString(5));
		ttt1_CNT = ttt1.getLong(6);
	}
	...
	private List<Object> getAIRowDataFromttt1() throws Exception{
		...
		row.add(ttt1.getObject(1));
		row.add(ttt1.getObject(2));
		row.add(ttt1.getObject(3));
		row.add(ttt1.getObject(4));
		row.add(ttt1.getObject(5));
		row.add(ttt1.getLong(6));
	...
	private void getChartDataAI_CHART_OBJ43( ) throws Exception{
		...
		if(!jip2.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(jip2.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromjip2();   //RecordSet jip2에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ43=AI_CHART_OBJ43 + jip2_IDX + "\t";

			//데이터 설정
			AI_CHART_OBJ43=AI_CHART_OBJ43 + jip2_CNT + "\r\n";
		}   //while end
	}
