...
class ReportHandler{
	...
	private void getScriptFromyyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			...
			//데이터 fetch
			if(cc.next()){
				setFieldVariableFromcc();	//ResultSet cc에서 필드 값 설정
			}else{
				setFieldInitFromcc();	//ResultSet cc의 필드 값 초기화
			}
	...
	private void getChartDataAI_CHART_OBJ32( ) throws Exception{
		...
		while(c.next()){   //데이터 루프문
			...
			//X축 레이블 필드 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + c_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + c_CNT + "\r\n";
		}   //while end
	}
