...
class ReportHandler{
	...
	private void getScriptFromyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet a3a3 객체의 생성
			if(a3a3==null){
				a3a3 = stat_a3a3.executeQuery();
			}else{
				a3a3.beforeFirst();
			}
			...
			//데이터 fetch
			if(a3a3.next()){
				setFieldVariableFroma3a3();	//ResultSet a3a3에서 필드 값 설정
			}else{
				setFieldInitFroma3a3();	//ResultSet a3a3의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//ResultSet a3 객체의 생성
		if(a3==null){
			a3 = stat_a3.executeQuery();
		}else{
			a3.beforeFirst();
		}

		//--while Loop Start--##
		while(a3.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFroma3();	//RecordSet a3에서 필드 값 설정
	...
	private void getChartDataAI_CHART_OBJ41( ) throws Exception{
		...
		if(!a3.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(a3.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFroma3();   //RecordSet a3에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a3_AREA + "\t";

			//데이터 설정
			AI_CHART_OBJ41=AI_CHART_OBJ41 + a3_CNT + "\r\n";
		}   //while end
	}
