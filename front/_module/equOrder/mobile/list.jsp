...
%>
	...
	<div class="MPTit MPTit02"><h2>예약정보</h2></div>

	<!--tabBox S-->
	<div class="tabBox">
		<ul>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("ALL") || util.getStr(paramMap.get("orderState")).equals("")) { %> class="on" <% } %> id="tabALL"><a href="javascript:orderTab('ALL');" title="전체 검색" >전체</a></li>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("1")) { %> class="on" <% } %>><a href="javascript:orderTab('1');" title="승인요청 검색" >승인요청</a></li>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("2")) { %> class="on" <% } %>id="tab2" class="end02"><a href="javascript:orderTab('2');" title="이용승인 검색">이용승인</a></li>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("3")) { %> class="on" <% } %>id="tab3" class="MA02"><a href="javascript:orderTab('3');" title="승인거절 검색">승인거절</a></li>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("4")) { %> class="on" <% } %>id="tab4" class="MA02"><a href="javascript:orderTab('4');" title="이용완료 검색">이용완료</a></li>
			<li <%if(util.getStr(paramMap.get("orderState")).equals("5")) { %> class="on end" <% } %>id="tab5" class="end MA02"><a href="javascript:orderTab('5');" title="승인취소 검색">승인취소</a></li>
		</ul>
	</div>
	<!--tabBox E-->
	...
	<div class="list_top">
		...
		<div class="b_srchBox">
			...
			<div class="labelBox01">
				...
				<div class="btw">~</div>
				<input type="text" class="txtbox MAL0" name="searchEdDt" id="searchEdDt" value="<%=util.getStr(paramMap.get("searchEdDt")) %>" size="10" title="예약신청 종류일 예시 2016-09-20" />
			
			</div>
			
			<div class="labelBox02">
			
				<label for="searchWord" class="label02">장비/시설명</label>
				<div class="b_srch">
					<!-- 키워드 -->
		...
		<div class="basic_listWrap MAT20">
			...
			%>
					...
					<tr style="cursor:pointer" onclick="pageViewGo('<%=rs.get("EQU_ORDER_IDX")%>');">
						...
						<th scope="col" class="r_line_bold">NO</th>
						...
						<td class=""><!-- 예약현황 -->
							...
							<!-- <img alt="승인요청" src="/img/back/comm/order_state_img_1.png" /> --><div class="br01">승인요청</div>
							<% } %> <%if(util.getStr(rs.get("ORDER_STATE")).equals("2")) { %>
							<!-- <img alt="이용승인" src="/img/back/comm/order_state_img_2.png" /> --><div class="yw01">이용승인</div>
							...
							<!-- <img alt="이용완료" src="/img/back/comm/order_state_img_4.png" /> --><div class="gr01">이용완료</div>
					...
					<tr style="cursor:pointer" onclick="pageViewGo('<%=rs.get("EQU_ORDER_IDX")%>');">
						...
						<td class="r_line_bold" rowspan="2">
							<%=cont%>
						</td>
						
						<th scope="col">신청자(신청일)</th>
						
						<td class="r_line_none"><!-- 신청자(신청일) -->
					...
					<tr style="cursor:pointer" onclick="pageViewGo('<%=rs.get("EQU_ORDER_IDX")%>');">
						<th scope="col">장비/시설명</th>
						<td class="r_line_none" colspan="4"><!-- 장비/시설명 -->
							<%=util.getStr(rs.get("KOR_NM"))%><br />(<%=util.getStr(rs.get("ENG_NM"))%>)
						</td>
					</tr>
					
					<!--tr>
						<td colspan="7" class="r_line_none bdBtm p10"></td>
					</tr-->
