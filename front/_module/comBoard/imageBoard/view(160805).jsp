			...
			<tbody>
			  ...
			  <tr>
			    <td class="r_line_none view_cont" colspan="4" style="word-break:break-all; overflow:hidden;">
			    	<div style="padding: 10px;"><%=dataMap.get("CONTENT").trim()%></div>
					...
					%>
							<div>
								<img src = "/file/<%=fileGrp+"/"+rs.get("TITLE")%>" style="max-width:1000px; vertical-align:middle" alt="이미지<%=imgCnt%>" />
							</div>
					<%
								imgCnt++;
...
<% }%>
		...
		<%if(util.loginCheck()) { %>
				...
				<button type="button" id="inpCmt" class="btn_rgt" style="float:right;width:12%;height:80px">등록</button>
			<% } %>
