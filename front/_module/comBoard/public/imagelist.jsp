	...
	<form action="sub.do" name="searchForm" id="searchForm" method="post" class="boardSearch">
		...
		<div class="board_count013">전체 <span class="blue1"><%=staticVO.getTotalCount()%></span>건</div>
			...
			<div class="b_btn_listBox013">
				<button type="button" class="b_btn_block013" onclick="goSubmit('list','image')" title="이미지리스트 현재페이지"><img src="/img/board/btn_block_ovr.png" alt="이미지리스트" /></button>
		...
		%>
			<li class="tooltip" id="_<%=num %>">
				...
				<!--<span class="down_publist">
					...
					<%--
					<a href="/cmm/fms/ComFileDown.do<%=fileParam.replace("boardIdx=", "dataIdx=") %>" title="첨부파일 다운로드" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('Image22','','/img/board/btn_down1_ovr.gif',1)">
				...
				<span id="tooltip_<%=num %>" class="tooltip_box">
					...
					<a href="sub.do?m=<%=m %>&amp;boardComIdx=<%=boardComIdx %>&amp;mode=view&amp;listMode=<%=listMode %>&amp;sortMode=<%=sortMode %>&amp;boardIdx=<%=util.getStr(rs.get("ARTICLE_ID"))%>&amp;nowPage=<%=nowPage%>&amp;siteId=<%=siteId%>" title="<%=util.getStr(rs.get("SUBJECT"))%> 상세보기">
						...
						<span class="tooltip_info" >
							발행기관(출처) :
							<%if(util.getStr(rs.get("DATA04")).length()>6){%>
								<%=util.getStr(rs.get("DATA04")).substring(0,6)+"···"%>
							<%}else{%>
