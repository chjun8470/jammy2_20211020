...
%>
 ...
 <form name="fview" id="fview" action="./sub.do" method="post" >
		...
		<input type="hidden" name="reUrl" id="reUrl" value="?<%=m%>&amp;boardComIdx=<%=afterBoardComIdx%>" />
	...
	<div class="basic_viewWrap">
		...
		<thead>
		  <tr>
		    <th scope="col">제목</th>
		    <td class="r_line_none b_notice" colspan="3"><span class="notice_blue">[공지]</span><%=util.getStr(dataMap.get("SUBJECT")) %></td>
		  </tr>
		</thead>
		<tbody>
		  <tr>
		  ...
		  <tr>
		    <td class="r_line_none view_cont" colspan="4"><%=String.valueOf(dataMap.get("CONTENT"))%></td>
		  </tr>
		</tbody>
		</table>
	...
	<!--basic_listWrap E-->
		...
		<%if(util.getStr(boardMap.get("PERM_CMT")).equals("Y")) { %>
			...
			<button type="button" id="inpCmt" class="btn_rgt2" style="float:right;width:80px;height:80px">답변</button>
		<% } %>
		<%if(util.loginCheck() && util.getBbsAuth(loginVO,boardMap,"EDITOR")){ %>
			<input type="button" class="btn_rgt" value="수정" onclick="goSubmit('noticeEdit')"/>
		<%}%>
		<%if(util.loginCheck() && util.getBbsAuth(loginVO,boardMap,"DEL")){ %>
			<input type="button" class="btn_rgt" value="삭제" onclick="goSubmit('noticeDel')"/>
		<%} %>
...
//<![CDATA[
	...
	function editCmt(no, idx, cmt) {
		...
		tag += "<td colspan='2'><textarea rows='5' cols='10' name='editComment'id='editComment' style='resize:none; width:100%; height:45px;'>"+ cmt +"</textarea></td>";
