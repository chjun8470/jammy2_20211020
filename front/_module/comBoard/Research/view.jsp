...
<table class="skin_view" summary="수요/만족도 조사 상세보기 테이블로 제목, 등록일, 조회, 첨부파일, 구글 설문조사 URL, 회신처, 기간, 기관명, 응답대상자, 내용으로 구성되어 있습니다.">
  <caption>수요/만족도 조사 상세보기 테이블</caption>
    <colgroup>
      <col style="width:15%;" />
      <col style="width:35%;" />
      <col style="width:15%;" />
  ...
  <tr>
    ...
    <%
			...
			<ul class="file" style="line-height: 22px;">
								...
								%>
									<li class="text" >
										<a href="/cmm/fms/ComFileDown.do<%=fileParam%>" title="첨부파일 다운로드"><%=util.getStr(rs.get("TITLE")) %> (<%=util.getStr(rs.get("FILE_SIZE")) %> Byte) </a>
			...
			<tr>
	              ...
	              <td  class="r_line_none" colspan="3">
	              	<% if ( !util.getStr(dataMap.get("DATA01")).equals("") ) { %>
		              	<a href="<%=util.getStr(dataMap.get("DATA01")) %>" target="_blank" title="새창열림" ><%=util.getStr(dataMap.get("DATA01")) %></a>
	              	<% }else{%>
						-
					<% } %>
	              </td>
		...
		<%if(util.getStr(boardMap.get("PERM_CMT")).equals("Y") && loginVO.getUserIdx()!=null){ %>
			<textarea title="의견 내용 입력창" rows="5" name="comment" class="b_txtbox4" style="width:644px;min-height:20px; height:78px;"></textarea>
			<button type="button" id="inpCmt" class="btn_rgt2 color_box_btn_2" style="float:right;width:80px;height:80px">등록</button>
		<% }%>
			...
			<%if(util.loginCheck() && util.getBbsAuth(loginVO,boardMap,"EDITOR") && ( util.getStr(loginVO.getAuthLv()).equals("99") ||
				...
				<input type="button" class="btn_rgt color_box_btn_1" value="수정" onclick="pageEditGo()"/>
			<% }%>
			...
			<%if(util.loginCheck() && util.getBbsAuth(loginVO,boardMap,"DEL") && ( util.getStr(loginVO.getAuthLv()).equals("99") ||
				...
				loginVO.getUserId().equals(dataMap.get("WRITER_ID"))   ) ){ %>
			<span>
				<input type="button" class="btn_rgt color_box_btn_1" value="삭제" onclick="pageDelGo()"/>
			</span>
			<% }  %>
