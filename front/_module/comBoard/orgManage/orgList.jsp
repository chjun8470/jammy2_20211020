...
%>
	...
	<div class="list_top006">
		...
		<div class="b_srchBox002">
			<div class="b_srch002" style="float:right;">
                <label class="hide" for="orgSearchWord">검색어를 입력하세요</label>
				<input id="orgSearchWord" class="b_srch_txt002" name="orgSearchWord"  value="<%=orgSearchWord%>" type="text" />
				<input type="image" src="/img/board/btn_ico_srch.png" class="b_go_btn002" alt="검색" />
			</div>
		</div>
	...
	<div id="cont_block" style="margin-top: 20px">
			...
			<table class="skin_list" summary="기관담당자 목록 테이블로 글번호, CI로고, 기관명, 대표전화, 담당자보기로 구성되어 있습니다.">
            <caption>기관담당자 목록</caption>
				<!-- colgroup s -->
			    <colgroup>
			     <col style="width: 10%;" />
				<col style="width: 20%;" />
				...
				<tbody>
					...
					<tr>
						...
						<a href="./sub.do?m=<%=m%>&amp;mode=list&amp;listMode=list&amp;orgSearchWord=<%=orgSearchWord%>&amp;searchWord=<%=searchWord%>&amp;orgGrpCd=<%=util.getStr(rs.get("ORG_GRP_CD"))%>" title="<%=util.getStr(rs.get("ORG_GRP_NM"))%> 기관 상세보기" >
						...
						<td class="r_line_none">
							<a href="./sub.do?m=<%=m%>&amp;mode=list&amp;listMode=list&amp;orgSearchWord=<%=orgSearchWord%>&amp;searchWord=<%=searchWord%>&amp;orgGrpCd=<%=util.getStr(rs.get("ORG_GRP_CD"))%>" title="<%=util.getStr(rs.get("ORG_GRP_NM"))%> 기관 상세보기" ><img src="/img/cont/icon_main04.png" alt="담당자보기" /></a>
						</td>
					</tr>
					<% cont--; } %>
				</tbody>
