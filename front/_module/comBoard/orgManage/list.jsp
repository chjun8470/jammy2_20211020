...
%>
	...
	<div class="list_top006">
		...
		<div class="b_srchBox006">
			...
			<div class="b_srch006">
				...
				<!-- 키워드 -->
				<input class="b_srch_txt006" type="text" name="searchWord" id="searchWord" value="<%=util.getStr(paramMap.get("searchWord"))%>" />
				<!-- 검색버튼 -->
				<input type="image" class="b_go_btn006" alt="검색" src="/img/board/btn_ico_srch.png" onclick="goSubmit('list')" />
			</div>
	...
	<div id="cont_block" style="margin-top: 20px">
			<table class="skin_list" summary="기관담당자 기관정보 테이블로 기관CI, 기관명으로 구성되어 있습니다.">
             <caption>기관담당자 기관정보</caption>
				<!-- colgroup s -->
			    <colgroup>
			      <col style="width:40%;" />
			      <col style="width:60%;" />
				...
				<tbody>
				  <tr>
				    <th scope="col" class="number">기관 CI</th>
				  ...
				  <tr>
				  	<td>
						...
						<%}else{ %>
							<img src="/img/no-image.gif" alt="이미지 없음" style="width:100px;height:100px;" />
						<%}%>
				  	</td>
				  	<td><%=orgGrpNm%></td>
				  </tr>
			...
			<table class="skin_list" summary="기관담당자 상세정보 테이블로 글번호, 부서명, 직위, 이름, 연락처로 구성되어 있습니다.">
              <caption>기관담당자 상세정보</caption>
				<!-- colgroup s -->
			    <colgroup>
			      <col style="width:10%;" />
			      <%-- <col style="width:30%;" /> --%>
				...
				<thead>
				  <tr>
				   ...
				   <!--  <th scope="col" class="b_notice">기관명</th> -->
				    <th scope="col" class="r_line_none2">부서명</th>
				    <th scope="col" class="r_line_none2">직위</th>
				    <th scope="col" class="lookup">이름</th>
				    <th scope="col" class="r_line_none file">연락처</th>
				  </tr>
				</thead>
				<!-- thead s -->
				<!-- tbody s -->
				<tbody>
					...
					<tr>
						...
						<td class="r_line_none2"><a href="./sub.do?m=<%=m%>&amp;mode=view&amp;listMode=list&amp;orgSearchWord=<%=orgSearchWord%>&amp;searchWord=<%=searchWord%>&amp;orgGrpCd=<%=orgGrpCd%>&amp;boardIdx=<%=util.getStr(rs.get("ARTICLE_ID"))%>" title="<%=util.getStr(rs.get("DATA02"))%> 상세보기" >
						...
						<td class="r_line_none file">
								<a href="./sub.do?m=<%=m%>&amp;mode=view&amp;listMode=list&amp;orgSearchWord=<%=orgSearchWord%>&amp;searchWord=<%=searchWord%>&amp;orgGrpCd=<%=orgGrpCd%>&amp;boardIdx=<%=util.getStr(rs.get("ARTICLE_ID"))%>" title="<%=util.getStr(rs.get("DATA02"))%> 상세보기" ><img src="/img/cont/ico_mid02_tab02.png" alt="담당자보기" /></a>
						</td>
					</tr>
					<% cont--; } %>
				</tbody>
	...
	<!--basic_listWrap E-->
		<div class="b_btn_area">
			...
			<!-- class="btn_lft2"  -->
				<a href="./sub.do?m=<%=m%>&amp;mode=list&amp;listMode=orgList&amp;orgSearchWord=<%=orgSearchWord%>"><input type="button" class="btn_rgt" value="이전으로"/></a>
		</div>
		<!--b_btn_area E-->
