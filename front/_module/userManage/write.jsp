...
<form name="fwrite" id="fwrite" action="/m/manageProc.do" method="post"
	...
	<input type="hidden" name="state" id="regId" value="접수중" />

	<div class="skin_bbs_write" style="margin-top: 30px">
		<font color="red">* 필수 입력사항입니다.</font>
		<table class="skin_basic_write"
			summary="작성하기로  제목, 태그, 첨부파일, 시스템설명(툴팁)을 제공합니다.">
					...
					<td>
						...
						<ul class="file" style="line-height: 22px;">
							...
							<li class="text"><input type="checkbox" name="fileFlog"
								...
								for="fileFlog_<%=fileCnt%>">파일삭제</label>&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="/biz/cmm/fms/ComFileDown.do<%=fileParam%>" title="첨부파일 다운로드"><%=util.deStr(rs.get("FILE_ORGN_NM"))%>
								...
								<div style="padding-top: 10px; display: none;"
									...
									<input type="file" name="<%=fileFullGrp%>File<%=fileCnt%>"
										id="<%=fileFullGrp%>File<%=fileCnt%>" title="파일첨부" /> <input
						...
						<div style="display: inline-block;"
							...
							<div>
								<input type="file" name="<%=fileFullGrp%>File<%=fileCnt%>"
									id="<%=fileFullGrp%>File<%=fileCnt%>" title="파일첨부" />
							</div>
						</div>
