...
<div class="list_top010">
		...
		if(!avg.equals("연도별")){
			if(selectValue.equals("구축장비수")){ %>
				<input type="image" name ="select" id ="eqcnt" src="../img/board/btn_order_build01_ovr.gif" alt = "구축장비수" value = "구축장비수" onclick='searchGo()'/>
				<input type="image" name ="select" id ="Price" src="../img/board/btn_order_build02.gif" alt = "구축금액" value = "구축금액" onclick='searchGo()'/>

			<%}else if(selectValue.equals("구축금액")){  %>
				<input type="image" name ="select" id ="eqcnt" src="../img/board/btn_order_build01.gif" alt = "구축장비수" value = "구축장비수" onclick='searchGo()'/>
				<input type="image" name ="select" id ="Price" src="../img/board/btn_order_build02_ovr.gif" alt = "구축금액" value = "구축금액" onclick='searchGo()'/>
			<%}
		}
			%>
		...
		<%if(!avg.equals("연도별")){ %>
			...
			<!--<label for="avgnm"><%if(avg.equals("기관별")){ %> 기관 이름 <%}else{ %>지역 이름<%} %></label>-->
				<div class="b_srch010">
					<input type="text" class="b_srch_txt010" id="avgnm" title="명칭" name ="avgnm" value="<%=avgnm%>" />
	...
	<div id="report_chart" style="min-width: 310px; height: 400px; margin: 0 auto"></div><br/>
		...
		<table class="skin_list" summary="게시판 목록을 나타내는 표입니다..">
			...
			for(HashMap rs:dayListTable) {%>
				...
				<%}else if(avg.equals("지역별")){%>
					<td class="r_line_none2"><%=util.getStr(rs.get("AREA"))%></td>
					<td class="number"><%=util.getComma(rs.get("AREA_LOCATION_CNT"))%></td>
					<td class="number"><%=util.getComma(rs.get("CNT"))%></td>
