...
<!--list_top S-->
	<div class="list_top">
		...
		<div class="publist_top_wrap">
			...
			<div class="b_srchBox" style="width:100%; margin-right:30px;">
				<select name="searchUseScopeCd" id="searchUseScopeCd" class="b_select" style="width:20%;">
				...
				<select name="searchUseScopeRange" id="searchUseScopeRange" class="b_select" style="width:20%;">
				...
				<select name="searchUseScopeMean" id="searchUseScopeMean" class="b_select" style="width:20%;">
				...
				<div class="b_srch" style="width:35%;">
					<!-- 키워드 -->
					<input type="text" name="searchWord" id="searchWord" class="b_srch_txt" value="<%=util.getStr(paramMap.get("searchWord")) %>" />
					<!-- 검색버튼 -->
					<input type="image" class="b_go_btn" src="/img/board/btn_ico_srch.png" onclick="pageSearchGo()" />
				</div>
			</div>
		</div>
	</div>
...
<!--list_top E-->
	...
	<div id="checkBoxArea">
		<input type="checkbox" id="checkAll" /><b>전체선택</b>
	</div>
					
	<div class="pub_list02" id="cont_block" style="padding:0px 30px 0px 0px;">
		...
		<ul class="pub_list_blog">
			...
			<li>
				...
				<ul class="select_subject">
					<input type="checkbox" id="checkContetns" name="checkContents" value="<%=util.getStr(rs.get("RND_EQU_IDX"))%>" style="margin:10px" /><b>선택</b>
				</ul>				
				...
				<div class="explanation02" style="padding:0px 30px 0px 0px;word-break:break-all;">
					...
					<p class="tit_publist_blog01" style="padding:0px 30px 0px 0px;word-break:break-all;">
					<%=util.getStr(rs.get("KOR_NM"))%>
					</p>
					<div class="txt_publist_blog01" style="padding:0px 30px 0px 0px;word-break:break-all;">
						<dl>
						<dt>제조사</dt><dd><%=util.getStr(rs.get("MANUFACTURE_NM"))%></dd>
						<%-- <dt>이용료</dt><dd><%if(util.getStr(rs.get("RENTAL_INFO")).equals("") || util.getStr(rs.get("RENTAL_INFO")).equals(null)) { %>
						정보없음
						<% } else { %>
							<%=util.getStr(rs.get("RENTAL_INFO"))%>
						<% } %></dd> --%>
						<dt>활용대상</dt><dd><%if(util.getStr(rs.get("USE_SCOPE_RANGE")).equals("") || util.getStr(rs.get("USE_SCOPE_RANGE")).equals(null)) { %>
						정보없음
						<% } else { %>
					...
					<div class="txt_publist_blog02" style="padding:0px 30px 0px 0px;word-break:break-all;">
				...
				<% } %>
					...
					<div class="b_btn_area3">
						<button id="favDelBtn" class="btn_lft" name="favDelBtn" onclick="favDeleteGo('<%=rs.get("EQU_GOODS_FAV_IDX")%>')">삭제</button>
					</div>
				</div>
			</li>
			<% cont--; } %> 
	...
	<!--  </form> -->
			...
			<div class="b_btn_area" style="width: 600px;">
				<input type="button" id="applyFav" name="applyFav" class="btn_rgt" value="예약신청" onclick="goApplyFav()" style="cursor: pointer"/>
			</div>
