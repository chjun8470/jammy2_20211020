...
<form id="fview" name="fview" method="get" action="/<%=request.getAttribute("SITE_ID") %>/sub.do">
	...
	<div class="b_ContBox">
		...
		<div class="basic_viewWrap">
			...
			<h4 class="title003"><%=util.getStr(dataMap.get("ORG_TYPE")).equals("2")?"기관":"기업" %> 기본정보</h4>
				<table class="skin_view2" summary="전남과학기술정보시스템 기업정보 조회 테이블로 기업/기관분류, 기관구분, 기관명, 소속지역, 소재지, 대표자명, 대표전화번호, 홈페이지, 기관관리자, 주소, 상세주소, 지도, 소개, 기관소개로 구성되어 있습니다.">
	  			<caption>전남과학기술정보시스템 기업정보 조회</caption>
					<colgroup>
					  <col style="width:15%;" />
					  <col style="width:35%;" />
					  <col style="width:15%;" />
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3">
							<input id="areaCd1" name="areaCd" type="checkbox" disabled="disabled" <%=util.getStr(dataMap.get("AREA_CD")).equals("1")?"checked='checked'":""%> value="1"/><label for="areaCd1">전라남도 기관</label>
							<input id="areaCd2" name="areaCd" type="checkbox" disabled="disabled" <%=!util.getStr(dataMap.get("AREA_CD")).equals("1")?"checked='checked'":""%> value="2"/><label for="areaCd2">지역외 기관</label>
						</td>
					</tr>
					<tr>
						<th scope="col">소재지</th>
					...
					<tr>
						...
						<td colspan="3">
							...
							<a href="/cmm/fms/ComFileDown.do?dataGrp=orgManage&amp;fileId=<%=util.getIntStr(dataMap.get("ORG_FILE_ID"))%>&amp;dataIdx=<%=util.getStr(paramMap.get("orgGrpCd"))%>" title="첨부파일 다운로드">
