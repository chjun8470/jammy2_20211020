...
<form name="fview" id="fview" action="sub.do" method="post" >
	...
	<divclass="skin_bbs_write" style="margin-top: 30px">
			...
			<tbody>
				...
				<tr id="profile1">
					<td rowspan="4" style="text-align: center; vertical-align: middle;">
						...
						<%}else{%>
							<img src = "/file/<%=fileGrp+"/"+dataMap.get("TITLE")%>" style="width:150px;height:180px;" alt="프로필사진" />
						<%}%>
					</td>
					<th scope="col" >이름</th>
			 	...
			 	<tr id="profile2" style="display:none">
					<td colspan="4" style="width:135px;height:150px; text-align: center; vertical-align: middle;">
						<%if(util.getIntStr(dataMap.get("FILE_ID")).equals("") || util.getIntStr(dataMap.get("FILE_ID")).equals(null)){ %>
							<imgstyle="width:135px;height:150px;" src="/img/main/no_image01.gif" alt="프로필사진" />
						<%}else{%>
				...
				<tr>
					<th scope="col" >전문가 구분</th>
					 <td class="r_line_none" colspan="4">
						...
						R&D <input type="checkbox" name="proJntisRnd" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_RND"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						기업자원 <input type="checkbox" name="proJntisCompany" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_COMPANY"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						교육 <input type="checkbox" name="proJntisEducation" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_EDUCATION"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
...
<%} %>
			...
			<%if(listSC.size()>0){ %>
				...
				<% for(HashMap rs:listSC) {%>
			 	<tr>
			 		...
			 		<td><%=util.getStr(rs.get("SC_MAJOR"))%></td>
		...
		<table class="skin_list textCenter">
			...
			<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="6">데이터가 존재하지 않습니다</td>
				</tr>
			<%} %>
		</table>
		...
		<table class="skin_list textCenter">
			...
			<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="5">데이터가 존재하지 않습니다</td>
				</tr>
			<%} %>
		</table>
		<h2 class="h4_tit">전문분야</h2>
		...
		<%if(listEX.size()>0){ %>
			...
			<tr>
				<td colspan="6" class="r_line_none">
						정책기획 <input type="checkbox" name="policyPlan" value="Y" <%=(util.getStr(rs.get("EX_POLICY_PLAN"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						연구개발 <input type="checkbox" name="researchDev" value="Y" <%=(util.getStr(rs.get("EX_RESEARCH_DEV"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						기업지원 <input type="checkbox" name="companySupport" value="Y" <%=(util.getStr(rs.get("EX_COMPANY_SUPPORT"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						지식재산 <input type="checkbox" name="jip" value="Y" <%=(util.getStr(rs.get("EX_JIP"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						연구장비 <input type="checkbox" name="jnsp" value="Y" <%=(util.getStr(rs.get("EX_JNSP"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						인력양성 <input type="checkbox" name="training" value="Y" <%=(util.getStr(rs.get("EX_POWER_TRAINING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						기반구축 <input type="checkbox" name="baseContracture" value="Y" <%=(util.getStr(rs.get("EX_BASE_CONSTRUCT"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						컨설팅 <input type="checkbox" name="consulting" value="Y" <%=(util.getStr(rs.get("EX_CONSULTING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>&nbsp;&nbsp;
						마케팅 <input type="checkbox" name="marketing" value="Y" <%=(util.getStr(rs.get("EX_MARKETING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled'/>
				</td>
			</tr>
			<tr>
		...
		<%
			...
			<tr>
				<tdstyle="text-align:center;"colspan="6" class="r_line_none">
						정책기획 <input type="checkbox" name="policyPlan" disabled='disabled'/>&nbsp;&nbsp;
						연구개발 <input type="checkbox" name="researchDev" disabled='disabled'/>&nbsp;&nbsp;
						기업지원 <input type="checkbox" name="companySupport" disabled='disabled'/>&nbsp;&nbsp;
						지식재산 <input type="checkbox" name="jip" disabled='disabled'/>&nbsp;&nbsp;
						연구장비 <input type="checkbox" name="jnsp" disabled='disabled'/>&nbsp;&nbsp;
						인력양성 <input type="checkbox" name="training" disabled='disabled'/>&nbsp;&nbsp;
						기반구축 <input type="checkbox" name="baseContracture" disabled='disabled'/>&nbsp;&nbsp;
						컨설팅 <input type="checkbox" name="consulting" disabled='disabled'/>&nbsp;&nbsp;
						마케팅 <input type="checkbox" name="marketing" disabled='disabled'/>
				</td>
			</tr>
			<tr>
				<th scope="col">전공계열</th>
		...
		<h2 class="h4_tit">연구분야</h2>
		<table class="skin_list textCenter">
			<caption>연구분야</caption>
			<colgroup>
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:40%" />
		 	...
		 	<tr>
		 		...
		 		<th scope="col" >중분류</th>
		 		<th scope="col" >소분류</th>
		 		<th scope="col"class="r_line_none">비고</th>
		 	</tr>
		 	<%
			if (listRE.size() > 0) {
			%>
			<% for(HashMap rs:listRE) {%>
		 	<tr>
				<td><%=util.getStr(rs.get("RE_DETH1_NAME"))%></td>
				<td><%=util.getStr(rs.get("RE_DETH2_NAME"))%></td>
				<td><%=util.getStr(rs.get("RE_DETH3_NAME"))%></td>
				<td class="r_line_none"><%=util.getStr(rs.get("RE_NOTE"))%></td>
		 	</tr>
		 	<%}%>
		<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="4">데이터가 존재하지 않습니다</td>
				</tr class="r_line_none">
		<%} %>
		</table>
		<h2 class="h4_tit">심사분야</h2>
		<table class="skin_list textCenter">
			<caption>심사분야</caption>
			<colgroup>
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:40%" />
		 	...
		 	<%if(listEV.size()>0){ %>
			<% for(HashMap rs:listEV) {%>
				<tr>
		 			<td><%=util.getStr(rs.get("EV_DETH1_NAME"))%></td>
		 			<td><%=util.getStr(rs.get("EV_DETH2_NAME"))%></td>
		 			<td><%=util.getStr(rs.get("EV_DETH3_NAME"))%></td>
		 			<td class="r_line_none"><%=util.getStr(rs.get("EV_NOTE"))%></td>
		 		</tr>
		 	<%}%>
		<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="4" class="r_line_none">데이터가 존재하지 않습니다</td>
				</tr>
		<%} %>
		</table>
		<h2 class="h4_tit">컨설팅분야</h2>
		<table class="skin_list textCenter">
			<caption>컨설팅분야</caption>
			<colgroup>
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:20%" />
			<col style="width:40%" />
			 ...
			 <%if(listCO.size()>0){ %>
			<% for(HashMap rs:listCO) {%>
				<tr >
		 			<td><%=util.getStr(rs.get("CO_DETH1_NAME"))%></td>
		 			<td><%=util.getStr(rs.get("CO_DETH2_NAME"))%></td>
		 			<td><%=util.getStr(rs.get("CO_DETH3_NAME"))%></td>
		 			<td class="r_line_none"><%=util.getStr(rs.get("CO_NOTE"))%></td>
			 	</tr>
			 <%} %>
		<%} else{%>
				<tr>
		...
		<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="8" class="r_line_none">데이터가 존재하지 않습니다</td>
				</tr>
		<%} %>
		</table>
		...
		<%} else{%>
				<tr>
					<tdstyle="text-align:center;"colspan="5" class="r_line_none">데이터가 존재하지 않습니다</td>
				</tr>
		<%} %>
		</table>
		<!-- 연구논문 실적 E -->
