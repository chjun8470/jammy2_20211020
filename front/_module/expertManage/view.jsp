...
<form name="fview" id="fview" action="sub.do" method="post" >
	...
	<h2>전문가 정보 상세보기</h2>
		...
		<table id="profileTable" class="skin_view" summary="기본정보 표입니다. 이름,성별,출생년도,이메일,국적 출생지역,전화번호,휴대전화 등을 확인합니다." >
				...
				<tr>
					<th scope="col" >전문가 구분</th>
					 <td class="r_line_none" colspan="4">
						<label for="proJntisPlanning">정책기획</label> <input type="checkbox" id="proJntisPlanning" name="proJntisPlanning" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_PLANNING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisRnd">연구개발</label> <input type="checkbox" id="proJntisRnd" name="proJntisRnd" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_RND"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisCompany">기업지원</label> <input type="checkbox" id="proJntisCompany" name="proJntisCompany" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_COMPANY"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJeinet">연구장비</label> <input type="checkbox" id="proJeinet" name="proJeinet" value="Y" <%=(util.getStr(dataMap.get("PRO_JEINET"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJipbank">지식재산</label> <input type="checkbox" id="proJipbank" name="proJipbank" value="Y" <%=(util.getStr(dataMap.get("PRO_JIPBANK"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisEducation">인력양성</label> <input type="checkbox" id="proJntisEducation" name="proJntisEducation" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_EDUCATION"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisConsulting">컨설팅</label> <input type="checkbox" id="proJntisConsulting" name="proJntisConsulting" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_CONSULTING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisMarketing">마케팅</label> <input type="checkbox" id="proJntisMarketing" name="proJntisMarketing" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_MARKETING"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
						<label for="proJntisEtc">기타</label> <input type="checkbox" id="proJntisEtc" name="proJntisEtc" value="Y" <%=(util.getStr(dataMap.get("PRO_JNTIS_ETC"))).equals("Y")? "checked='checked'" : "" %> disabled='disabled' />&nbsp;&nbsp;
					</td>
				</tr>
<%} %>
				<tr>
		...
		<table class="skin_list textCenter" summary="학력정보 표입니다.학교명,소재지,취득학위,재학기간,학력정보 등의 정보를 확인합니다." >
		...
		<table class="skin_view textCenter"  summary="전공분야 표입니다. 전공계열,복수전공계열 등의 정보를 입력할수 있습니다." >
		...
		<%if(listEX.size() > 0){ %>
			...
			<tr>
				...
				<td ><%=util.getStr(String.valueOf(rs.get("EX_MAJOR_NAME")))%></td>
				<th scope="col" >전공분야</th>
				<td><%=util.getStr(String.valueOf(rs.get("EX_REALM_NAME")))%></td>
				<th scope="col" >세부전공</th>
				<td class="r_line_none"><%=util.getStr(String.valueOf(rs.get("EX_DETAIL_NAME")))%></td>
			</tr>
			<tr>
				<th scope="col" >복수전공계열</th>
				<td><%=util.getStr(String.valueOf(rs.get("EX_MAJOR_PLURAL_NAME")))%></td>
				<th scope="col" >전공분야</th>
				<td><%=util.getStr(String.valueOf(rs.get("EX_REALM_PLURAL_NAME")))%></td>
				<th scope="col" >세부전공</th>
				<td class="r_line_none"><%=util.getStr(String.valueOf(rs.get("EX_DETAIL_PLURAL_NAME")))%></td>
			</tr>
		<%
			}
		} else{%>
		...
		<table class="skin_list textCenter" summary="경력정보 표입니다. 직장명,소재지,재직기간,최종직위,주요업무 등의 정보를 확인합니다." >
		...
		<table class="skin_list textCenter" summary="대외활동 표입니다. 조직(학회)명, 참여기간,직위,주요업무 등을 확인합니다." >
		...
		<table class="skin_list textCenter" summary="연구개발과제(프로젝트) 수행실적 표입니다. 부처/기관/발주처명, 사업명,과제명,총사업참여기간,사업비,참여형태 등을 확인합니다." >
		...
		<table class="skin_list textCenter" summary="외부평가 수행실적 표입니다. 평가대상 사업명,요청기관명,평가기간,평가내용 등을 확인합니다."  >
		...
		<table class="skin_list textCenter" summary="컨설팅 수행실적 표입니다. 컨설팅 사업명,요청기관명,컨설팅기간,컨설팅 내용 등을 확인합니다." >
		...
		<table class="skin_list textCenter" summary="연구논문 실적표입니다.연번,논문명,논문유형,학술지명,게제년도 등을 확인합니다." >
		...
		<table class="skin_list textCenter" summary="지식재산 실적 표입니다. 연번,지식재산명,지식재산유형,인증/발행처,등록년도 등을 확인합니다." >
		...
		<table class="skin_list textCenter" summary="수상실적 표입니다. 연번,훈격,수여일자,수여기관,공적내용 등을 확인합니다." >
		...
		<table class="skin_list textCenter"summary="자격사항 표입니다.연번,취득일,발급기관명,자격구분,자격명등을 확인합니다." >
