...
<%@ page import="java.io.*,java.util.*,java.util.regex.*,java.text.*,java.net.*" %>
	...
	<div id="MPleftMenu">
		<a class="mp_btn_home" href="/sso/logout.do?siteId=mjeinet">LOGOUT</a>
		...
		<ul class="MPleftMenuList">
			<li id="MPleft01" <%if("1".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/mypagecon.do" title="회원가입">정보관리</a> --><a href="javascript:void(0);" class="MPleftMenu" title="회원가입">정보관리</a>
				<div id="MPleftSub01" <%if(!"1".equals(menu1)){%>class="DisplayNone"<%} %> >
					<ul>
						<li id="MPleftSubImg0101" <%if("1".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/mypagecon.do?siteId=mjeinet" title="정보수정">정보수정</a></li>
						<li id="MPleftSubImg0103" <%if("1".equals(menu1) && "2".equals(menu2)){%> class="on" <%}%>><a href="/sso/passChange.do?siteId=mjeinet" title="비밀번호변경">비밀번호변경</a></li>
						<li id="MPleftSubImg0102" <%if("1".equals(menu1) && "3".equals(menu2)){%> class="on" <%}%>><a href="/sso/leave.do?siteId=mjeinet" title="회원탈퇴">회원탈퇴</a></li>
						<li id="MPleftSubImg0104" <%if("1".equals(menu1) && "4".equals(menu2)){%> class="on" <%}%>><a href="/sso/agecon.do?siteId=mjeinet" title="기관변경">기관변경</a></li>
					</ul>
				</div>
			</li>
			...
			<li id="MPleft02" <%if("2".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/equorder.do" class="MPleftMenu" title="장비예약">장비예약</a> --><a href="javascript:void(0);" class="MPleftMenu" title="장비예약">장비예약</a>
				<div id="MPleftSub02" <%if(!"2".equals(menu1)){%>class="DisplayNone" <%} %> >
					<ul>
						<li id="MPleftSubImg0201" <%if("2".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/equorder.do?siteId=mjeinet" title="예약정보">예약정보</a></li>
						<li id="MPleftSubImg0202" <%if("2".equals(menu1) && "2".equals(menu2)){%> class="on" <%}%>><a href="/sso/myfav.do?siteId=mjeinet" title="장바구니">장바구니</a></li>
					</ul>
				</div>
			</li>
			...
			<!--<li id="MPleft03" <%if("3".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/education.do" title="교육정보">교육정보</a><a href="javascript:void(0);" class="MPleftMenu" title="교육정보">교육정보</a>
				<div id="MPleftSub03" <%if(!"3".equals(menu1)){%>class="DisplayNone" <%} %> >
					<ul>
						<li id="MPleftSubImg0301" <%if("3".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/education.do?siteId=mjeinet" title="전체교육일정">전체교육일정</a></li>
					</ul>
				</div>
			</li>
			<li id="MPleft04" <%if("4".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/event.do" title="행사정보">행사정보</a><a href="javascript:void(0);" class="MPleftMenu" title="행사정보">행사정보</a>
				<div id="MPleftSub04" <%if(!"4".equals(menu1)){%>class="DisplayNone" <%} %> >
					<ul>
						<li id="MPleftSubImg0401" <%if("4".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/event.do?siteId=mjeinet" title="행사정보">행사정보</a></li>
					</ul>
				</div>
			</li>
		<%if(util.getInt(loginVO.getAuthLv()) >= 70){%>
				<li id="MPleft05" <%if("5".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/changeboard.do" title="담당자게시판">담당자게시판</a></li><a href="javascript:void(0);" class="MPleftMenu" title="담당자게시판">담당자게시판</a>
					<div id="MPleftSub05" <%if(!"5".equals(menu1)){%>class="DisplayNone" <%} %> >
						<ul>
							<li id="MPleftSubImg0501" <%if("5".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/changeboard.do?siteId=mjeinet" title="지식재산창출상담">담당자게시판</a></li>
						</ul>
					</div>
				</li>
		<%}%>
			...
			<li id="MPleft06" <%if("6".equals(menu1)){%> class="on" <%} %> ><!-- <a href="/sso/advice_01.do" title="상담게시판">상담게시판</a><a href="javascript:void(0);" class="MPleftMenu" title="상담게시판">상담게시판</a>
				<div id="MPleftSub06" <%if(!"6".equals(menu1)){%>class="DisplayNone" <%} %> >
					<ul>
						<li id="MPleftSubImg0601" <%if("6".equals(menu1) && "1".equals(menu2)){%> class="on" <%}%>><a href="/sso/advice_01.do?siteId=mjeinet" title="지식재산창출상담">지식재산창출상담</a></li>
						<li id="MPleftSubImg0602" <%if("6".equals(menu1) && "2".equals(menu2)){%> class="on" <%}%>><a href="/sso/advice_02.do?siteId=mjeinet" title="기술이전상담">기술이전상담</a></li>
						<li id="MPleftSubImg0603" <%if("6".equals(menu1) && "3".equals(menu2)){%> class="on" <%}%>><a href="/sso/advice_03.do?siteId=mjeinet" title="창업상담">창업상담</a></li>
						<li id="MPleftSubImg0604" <%if("6".equals(menu1) && "4".equals(menu2)){%> class="on" <%}%>><a href="/sso/advice_04.do?siteId=mjeinet" title="장비이용상담">장비이용상담</a></li>
						<li id="MPleftSubImg0605" <%if("6".equals(menu1) && "5".equals(menu2)){%> class="on" <%}%>><a href="/sso/advice_05.do?siteId=mjeinet" title="멘토상담">멘토상담</a></li>
	...
	<%-- <div id="MPleftMenu">
		...
		<div class="top">
			<a href="/jeinet/main.do" class="home">HOME</a>
	...
	%>
		<h1 class="mypage_<%if(paramMap.get("pageMode").equals("login")){%> MAL0<%}%>"><a href="/sso/home.do?siteId=mjeinet">마이페이지<br><span>MY PAGE</span></a></h1>
		<%if(!"".equals(util.getStr(loginVO.getUserIdx()))) {%>
			<div class="LogOut"><a href="/mjeinet/main.do" target="_self">
	...
	%>
		<h1 class="ci<%if(paramMap.get("pageMode").equals("login")){%> MAL0<%}%>"><a href="/sso/home.do?siteId=mjeinet">전남과학기술진흥센터</a></h1>
		<%if(!"".equals(util.getStr(loginVO.getUserIdx()))) {%>
			<div class="LogOut" ><a href="/sso/logout.do?siteId=mjeinet" target="_self"><img src="/img/common/btn_logout.png" alt="로그아웃" /></a></div>
		<%} %>
	<%
	}
	%>
