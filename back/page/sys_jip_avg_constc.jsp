...
%>
 	...
 	<div class="content_center" id="123123">
...
<button type="button" onclick="imageDownload();">Download</button>
...
function imageDownload(){
      html2canvas(document.getElementById("123123")).then(function(canvas) {//아이디로 원하는 영역 지정
         var aTag = document.createElement("a"); //a태그 생성
         var image = canvas.toDataURL("image/png").replace("image/png", "image/octet-stream"); //image 생성
         aTag.download="image.png"; //a태그에 download 속성 설정(파일명)
         aTag.setAttribute("href", image); //a태그에 다운로드할 이미지 설정
         aTag.click(); //a태그 click 이벤트 실행하여 다운로드
      });
}
