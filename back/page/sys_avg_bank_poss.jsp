   ...
   <br/>
		...
		<div class="leftMenuBox"  >
   		...
   		<div class="tabBox" style="margin-top:30px;display:none;" id="sho" >
			<ul id =tabul>
				<li id="ta1"  style="float:left;margin-left:10px;" onclick="pageShow('1')"><a href='javascript:void(0);'>보유건수</a></li>
				<li id="ta2"  style="float:left;margin-left:10px;" onclick="pageShow('2')"><a href='javascript:void(0);'>권리유형별</a></li>
			</ul>
	...
	<%}else{ %>	
		<div class="tabBox" style="margin-top:30px;">
			<ul id =tabul style="width: 1000px;">
				<li class="<%if(tap.equals("1")){ %>on<%}%>" style="float:left;margin-left:10px;"><a href="/sys/sys_avg_bank_poss.do?tap=1">보유건수</a></li>
				<li class="<%if(tap.equals("2")){ %>on<%}%>" style="float:left;margin-left:10px;"><a href="/sys/sys_avg_bank_poss.do?tap=2">권리유형별</a></li>
			</ul>
		</div>
   		<div class="btn_right_box">
			<input type="button" class="btn_inp_w_01" style="float: right;" value="인쇄" onclick="print()"/>
		...
		<%if(tap.equals("1")){ %>
	   <div>
	      <iframe src="/AIViewer55/temp/poss1.jsp?reportParams=showMenuBar:false&orgcd=<%=loginVO.getOrgGrpCd() %>" id="tem" name="tem" width="100%" height="100%" style="border:0px;" onload="autoReportResize('tem')"></iframe>
	   </div>
	   <%}else if(tap.equals("2")){ %>
	   <div>
	      <iframe src="/AIViewer55/temp/poss2.jsp?reportParams=showMenuBar:false&orgcd=<%=loginVO.getOrgGrpCd() %>" id="tem" name="tem"  style="border:0px;width:100%; height:100%" onload="autoReportResize('tem')"></iframe>
	   </div>
	   <%}%>
	<%} %>   
   </div>
...
//<![CDATA[
	...
	function avgShow(code){
		var url = "/AIViewer55/temp/poss1.jsp?reportParams=showMenuBar:false&orgcd="+code+"&dvsn=true" ;
		$("#codeValue").val(code);
		if(code!=null){
			$('#ta1').removeClass("on");
			$('#ta2').removeClass("on");
	...
	function pageShow(tap){
		...
		var url = "/AIViewer55/temp/poss"+tap+".jsp?reportParams=showMenuBar:false&orgcd="+code;
		$('#ta1').removeClass("on");
		$('#ta2').removeClass("on");
		var code1=tap;
		for(i=1;i<3;i++){
