...
%>
   ...
   <img style="width: 100%;" alt="준비중인 페이지입니다." src="/img/sub/sub_dsc.jpg" />
   		<%-- <div class="tabBox" style="margin-top:30px;">
			<ul id =tabul>
				<li class="<%if(tap.equals("1")){ %>on<%}%>" style="float:left;margin-left:10px;"><a href="/sys/sys_avg_cnstc_status.do?tap=1">기관별 구축 현황</a></li>
				<li class="<%if(tap.equals("2")){ %>on<%}%>" style="float:left;margin-left:10px;"><a href="/sys/sys_avg_cnstc_status.do?tap=2">지역별 구축 현황</a></li>
			</ul>
		</div>
		...
		<%if(tap.equals("1")){ %>
	   <div>
	      <iframe src="/AIViewer55/temp/status2.jsp?reportParams=showMenuBar:false" id="tem" name="tem" width="100%" height="100%" style="border:0px;" onload="autoReportResize('tem')"></iframe>
	   </div>
	   <%} else if(tap.equals("2")){ %>
	   <div>
	      <iframe src="/AIViewer55/temp/status1.jsp?reportParams=showMenuBar:false" id="tem" name="tem" width="100%" height="100%" style="border:0px;" onload="autoReportResize('tem')"></iframe>
	   </div>
	   <%} %> --%>
