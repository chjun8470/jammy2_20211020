			...
			if(dataList.size()>0){
				for(HashMap rs:dataList) {
					...
					<tr>
						...
						<td><button type="button" onclick="clickToBtn('<%=name%>','<%=util.getStr(rs.get("FIXED_ASET_NO"))%>','<%=util.getStr(rs.get("OPER_CHARGER"))%>',
						'<%=util.getStr(String.valueOf(rs.get("RND_EQU_IDX"))) %>', '<%=util.getStr(rs.get("OWN_ORG_NM")) %>', '<%=util.getStr(rs.get("OWN_ORG_SUB_NM")) %>',
						'<%=util.getStr(rs.get("OPER_POSITION")) %>', '<%=util.getStr(rs.get("OPER_TEL")) %>', '<%=util.getStr(rs.get("OPER_EMAIL")) %>')">선택</button></td>
					</tr>
				<%
				cont--;
				}
...
<script type="text/javascript">
	...
	function clickToBtn(val,val2,val3,val4, org, dept, position, tel, email){
		var obj = new Object();
		obj['equ_nm'] = val;
		obj['fixed_aset_no'] = val2;
		obj['psn_nm'] = val3;
		obj['rnd_equ_idx'] = val4;
		obj['pop_mode'] = "equ";
		opener.setPopup(obj);
		$("#equUserOrg", opener.document).val(org);
		$("#equUserDept", opener.document).val(dept);
		$("#equUserPosition", opener.document).val(position);
		$("#equUserPhone", opener.document).val(tel);
		$("#equUserEmail", opener.document).val(email);
		window.close();
	}
</script>
