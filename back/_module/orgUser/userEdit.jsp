...
%>
<form id = "fview" name = "fview" method="post" action="/sys/sys_org_user.do">
				...
				<div class="box_in">
					<table class="skin_basic_write" summary="회원정보를 나타내는 표 입니다">
	                	...
	                	<tr>
                			...
                			<td><%=util.getStr(dataMap.get("REG_DT"))%></td>
                			<th scope="col"><label for="regIp">등록아이피</label></th>
                			<td><%=util.getStr(dataMap.get("REG_IP"))%></td>
                		</tr>
                		<tr>	
                			<th scope="col"><label for="state">상태</label></th>
<%--                 			<td colspan="3"><%=util.getStr(dataMap.get("USER_APP_STATE"))%></td> --%>
                			<td colspan="3">
                			<select id="userAppState" name="userAppState" onchange="changeState(this.value)">
                				<option value="10" <%=util.getStr(dataMap.get("USER_APP_STATE")).equals("요청")? "selected='selected'" : "" %>> 요청 </option>
                				<option value="20" <%=util.getStr(dataMap.get("USER_APP_STATE")).equals("승인")? "selected='selected'" : "" %>> 승인 </option>
                				<option value="30" <%=util.getStr(dataMap.get("USER_APP_STATE")).equals("반려")? "selected='selected'" : "" %>> 반려 </option>
                			</select>
                			</td>
                		</tr>
                		<tr id="trAcceptNo" <%if(!util.getStr(dataMap.get("USER_APP_STATE")).equals("반려")){ %> style="display:none" <% }%>>
                			<th scope="col"><label for="">반려사유</label></th>
                			<td colspan="3"><textarea id="acceptNo" name="acceptNo" style="width:500px; height:100px"><%=util.getStr(dataMap.get("ACCEPT_NO"))%></textarea></td>
                		</tr>
                    	<tr>
...
<%--                 			<td><%=util.getStr(dataMap.get("BLNG_ORG_GUBUN"))%></td> --%>
              ...
              <div class="btn_box" >
	                	<div class="btn_right_box" >
	                		...
	                		<button class="btn_inp_w_01" type="button" onclick="pageDelGo()">삭제</button>
