...
<form id="fwrite" name="fwrite" method="post" action="/sys/sysCompanyManageProc.do" enctype="multipart/form-data" >
	<input type="hidden" id="mode" name="mode" value="write" />
	<input type="hidden" name="authCd" value="2" id="authCd2"/>
	<input type="hidden" id="orgType1" name="orgType" value="1" />
				...
				<div class="fr">
					...
					<table class="skin_basic_write">
						...
						<tbody>
							...
							<tr>
								<th scope="col">
									<label for="orgGrpNm"><span class="required">*</span>기업명</label>
								</th>
...
var emailPattern = /[\w]*@([0-9a-zA-Z][\-\w]*[0-9a-zA-Z]\.)+[a-zA-Z]{2,9}/;
	...
	function pageListGo() {
		$('#mode').val('list');
		$('#applyCd').val('');
		$('#fwrite').attr("action","/sys/sys_company_manage.do");
		$('#fwrite').submit();
	}
           ...
           searchAddrFromCoords(labMap.getCenter(), displayCenterInfo);
			...
			function submitGo(){
					...
					if($("#addr1").val()==null || $("#addr1").val()==""||$("#zipcode").val()==null || $("#zipcode").val()==""){
						errMsg+="기업주소 : 필수입력사항입니다."+"<br/>";
					}
					if($("#addr2").val()==null || $("#addr2").val()==""){
						errMsg+="기업상세주소 : 필수입력사항입니다."+"<br/>";
					}
					...
					var fileName2 = $('#orgManageFile1').val(); //기업 소개자료
					...
					if(fileName2 != "" && fileName2 != null){
						...
						if(infPattern.indexOf(fileName2)==-1){
							errMsg+="기업 소개자료는 PDF, 한글, 파워포인트, 워드 확장자 파일만 업로드가능합니다."+"<br/>";
							//$('#orgManageFile1').val("");
						}/*else if(this.files[0].size<300000){
							alert("300kb 이상의 이미지를 등록해주세요.");
							$('#jnsptotalFile'+fileCount).val("");
					...
					///////////////
					if(errMsg==""){
						$('#fwrite').attr("action", "/sys/sysCompanyManageProc.do");
						fwrite.submit();
					}else{
