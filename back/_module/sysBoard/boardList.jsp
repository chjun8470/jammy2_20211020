...
<div class="skin_list">
	<div>
		 <form action="./<%=myPage %>" name="searchForm" id="searchForm" method="post" class="boardSearch">
			...
			<input type="hidden" name="boardComIdx" id="boardComIdx" value="" />
			...
			<div class="search_box" style="padding:10px">
				<div class="board-search">
					...
					<!-- 키워드 -->
					<input class="board-input" type="text" name="searchWord" id="searchWord" value="<%=util.getStr(paramMap.get("searchWord"))%>" style="width:300px; height:25px" />
		...
		<table class="skin_basic_list">
			...
			%>
				<tr  onclick="goSubmit('list','<%=util.getStr(rs.get("BOARD_ID"))%>')" onmouseover="this.style.background='#f3f3f3'" onmouseout="this.style.background='white'">
					<td>
						<%=cont%>
					</td>
					<td>
...
<script type="text/javascript">
	function goSubmit(mode,idx){
		...
		if(mode=="list"){
			$('#nowPage').val(1); //게시판 처음 페이지 값은 '1'로 고정
			$('#boardComIdx').val(idx);
			$("#searchForm").attr("action", "/sys/sys_com_board.do");
		}
