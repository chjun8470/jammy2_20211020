...
<%
	...
	ArrayList<HashMap<String, String>> userEquList = request.getAttribute("userEquList") == null ? new ArrayList<HashMap<String, String>>(): (ArrayList<HashMap<String, String>>)request.getAttribute("userEquList");
...
// 	String fileFullGrp =fileGrp+fileSubGrp;
	...
	String userTp3 = util.getStr(dataMap.get("USER_TP"));

%>
<form id = "fview" name = "fview" method="post" action="/sys/sys_user.do" >
	...
	<input type="hidden" name="userTp3" id="userTp3" value="<%=userTp3%>" />
	...
	<div class="skin_bbs_write" style="margin-top: 30px">
		<table class="skin_basic_write" >
			...
			<tbody id="userTr">
				...
				<%if(!orgGrpCd.equals("") && !orgGrpCd.equals(null)){ %>
				<tr>
					<th scope="row" class="tit" colspan="4">기관정보
						<button class="btn_inp_w_01" type="button" onclick="location.href='/sys/sys_org_manage.do?orgGrpCd=<%=orgGrpCd%>&mode=view'">상세보기</button>
					</th>
				</tr>
				<tr>
				...
				<tr>
					<th scope="row" class="tit">담당장비</th>
					<td colspan="3">
						<%for(HashMap rs:userEquList) {%>
							<a href="/sys/sys_jnsptotal_eq.do?mode=view&rndEquIdx=<%=util.getStr(rs.get("RND_EQU_IDX"))%>"><%=util.getStr(rs.get("KOR_NM"))%>(<%=util.getStr(rs.get("ENG_NM"))%>)</a><br>
						<%} %>
					</td>
				</tr>
...
<%if(!util.getStr(dataMap.get("PRO_STATE")).equals("") && !util.getStr(dataMap.get("PRO_STATE")).equals(null)){ %>
				<tr>
					<th scope="row" class="tit" colspan="4">전문가정보
						<button class="btn_inp_w_01" type="button" onclick="location.href='/sys/sys_expert_manage.do?mode=view&userIdx=<%=util.getIntStr(dataMap.get("USER_IDX"))%>'">상세보기</button>
					</th>
				</tr>
				<tr>
					<th scope="row" class="tit">승인여부</th>
					<td colspan="3">
						<%=util.getStr(dataMap.get("PRO_STATE"))%>
					</td>
				</tr>
<%} %>
			...
			<%}%>
              <div class="btn_box" >
	                	<div class="btn_right_box" >
	                		...
	                		<button class="btn_inp_b_01" type="button" onclick="goDel('del','<%=util.getIntStr(dataMap.get("USER_IDX")) %>')">삭제</button>
