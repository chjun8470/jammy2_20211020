	...
	<div>
		<form action="./<%=myPage%>" name="searchForm" id="searchForm" method="post" class="boardSearch">
			...
			<div class="search_box" style="padding:10px">
				<div class="board-search">
					...
					<div>
						<label>사업년도</label>
						<input type="text" name="searchYear" id="searchYear" style="width:80px; height:20px" value="<%=util.getStr(paramMap.get("searchYear"))%>" />
						<select name="searchCen" id="searchCen" class="select" style="height: 26px">
							<option value="">::담당기관선택::</option>
							<option value="1" <%if("1".equals(util.getStr(paramMap.get("searchCen")))) {%> selected="selected" <%}%>>중앙행정기관</option>
							<option value="2" <%if("2".equals(util.getStr(paramMap.get("searchCen")))) {%> selected="selected" <%}%>>지자체</option>
						</select>
					</div>
		...
		<table class="skin_basic_list">
			...
			<tbody>
				...
				%>
				<tr>
					<td><img src="/img/board/ico_notice.png" alt="공지 아이콘" /></td>
					<td class="subject">
				...
				<tr>
					<td><%=cont%></td>
					<td><%if(util.getStr(rs.get("DATA01")).equals("1")){%>중앙행정<%}else{%>지자체<%}%></td>
					<td class="subject">
