...
<form name="fedit" id="fedit" action="<%=myPage%>" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="checkBox" id="checkBox" value="<%=util.getStr(dataMap.get("NOTICE_AT"))%>" />
	<input type="hidden" name="afterBoardComIdx" id="afterBoardComIdx" value="<%=beforeBoardComIdx%>" />
	...
	<div class="skin_bbs_write" style="margin-top: 30px">
		...
		<table class="skin_basic_write" summary="작성하기로  제목, 태그, 첨부파일, 시스템설명(툴팁)을 제공합니다.">
			...
			<tbody>
				<%if(util.getStr(boardMap.get("BOARD_ID")).equals("00005") || util.getStr(boardMap.get("BOARD_ID")).equals("00001")){ %>
				<tr>
          	      <th scope="row" class="tit">
      	             <label for="notice">공지글여부</label>
                  </th>
                  <td>
                  	사용 <input type="checkbox" name="noticeAt" value="Y" onclick="check_only(this)"/>
                  	미사용 <input type="checkbox" name="noticeAt" value="N" onclick="check_only(this)"/>
                  </td>
              	</tr>
              	<%}%>
				<tr>
...
<%-- 									<%=util.deStr(rs.get("FILE_ORGN_NM"))%> (<%=util.getStr(rs.get("FILE_SIZE"))%> Byte) --%>
		...
		<div class="btn_right_box" style="text-align: right; width: 980px;">
			<input type="button" class="btn_inp_b_01" value="수정" onclick="goSubmit('noticeEditProc')"/>
			<input type="button" class="btn_inp_w_01" value="취소" onclick="goSubmit('noticeView')"/>
		</div>
		<!--// bo_btn -->
	</div>
</form>
