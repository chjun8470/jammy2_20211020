	...
	<div class="skin_bbs_write" style="margin-top: 30px; width: 1000px;">
		...
		<table class="skin_basic_write" summary="작성하기로  제목, 태그, 첨부파일, 시스템설명(툴팁)을 제공합니다.">
			...
			<tbody>
				...
				<tr>
					<th class="tit">
						<font color="red">*</font><label for="state10"> 승인상태</label>
					</th>
					<td>
						<label><input type="radio" onchange="reason1(this.value)" name="state" id="state10" value="10" <%if("10".equals(util.getStr(dataMap.get("DATA15")))) {%> checked="checked" <%}%>><span class="btn_y_01" style="cursor:pointer; background-color:#FAED7D;">승인요청</span></label>
						<label><input type="radio" onchange="reason1(this.value)" name="state" id="state20" value="20" <%if("20".equals(util.getStr(dataMap.get("DATA15")))) {%> checked="checked" <%}%>><span class="btn_y_01" style="cursor:pointer; background-color:#73bb1e;">승인</span></label>
						<label><input type="radio" onchange="reason1(this.value)" name="state" id="state30" value="30" <%if("30".equals(util.getStr(dataMap.get("DATA15")))) {%> checked="checked" <%}%>><span class="btn_y_01" style="cursor:pointer; background-color:#FFBC9B;">반려</span></label>
					</td>
				</tr>				
				...
				<tr>
					...
					<td class="r_line_none p_top_btm_6">
						<span id="orgGroupTd">
							<input type="text" name="OwnOrgNm" id="OwnOrgNm" value="<%=util.getStr(dataMap.get("DATA07"))%>" class="inp_text b_txtbox1" style="padding-left:2px; font-size:12px" placeholder=" 기관검색버튼을 클릭해주세요."/>&nbsp;
							<input type="hidden" name="OwnOrg" id="OwnOrg" value="<%=util.getStr(dataMap.get("DATA17"))%>"/>
							<% if( util.getStr(loginVO.getAuthLv()).equals("99") ){ %>
								<input type="button" name="blngOrgNm" value="기관검색" id="blngOrgNm" class="btn_inp_b_01" onclick="popupWindow('registrationOrgList')" >
							<% } %>
						</span>
						<span>
							<input type="text" name="OwnDeptNm" id="OwnDeptNm" value="<%=util.getStr(dataMap.get("DATA08"))%>" class="inp_text b_txtbox1" style="padding-left:2px; font-size:12px" placeholder=" 부서검색버튼을 클릭해주세요."/>&nbsp;
							<input type="button" name="blngOrgSubNm" value="부서검색" id="blngOrgSubNm" class="btn_inp_b_01" onclick="popupWindow('registrationOrgSubList')" >
						</span>
					</td>
				</tr>
				<tr>
					...
					<td class="r_line_none p_top_btm_6">
						<table>
						<tr><th>성       명</th><td><input type="text" name="proChargeNm" id="proChargeNm" value="<%=util.getStr(dataMap.get("DATA10"))%>" class="inp_text b_txtbox1" /></td></tr>
						<tr><th>직       위</th><td><input type="text" name="proChargePosition" id="proChargePosition" value="<%=util.getStr(dataMap.get("DATA11"))%>" class="inp_text b_txtbox1" /></td></tr>						
						<tr><th>전       화</th><td><input type="text" name="proChargeTel" id="proChargeTel" value="<%=util.getStr(dataMap.get("DATA12"))%>" class="inp_text b_txtbox1" maxlength="11"/></td></tr>
						<tr><th>이  메  일</th><td><input type="text" name="proChargeEmail" id="proChargeEmail" value="<%=util.getStr(dataMap.get("DATA13"))%>" class="inp_text b_txtbox1" /></td></tr>
						</table>						
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					%>
							...
							<div style="display: inline-block;"
								...
								<div>
									<input type="file" class="inp_text" name="<%=fileFullGrp%>File1" id="<%=fileFullGrp%>File1" title="파일첨부" />
								</div>
							</div>
