		...
		<form action="./<%=myPage%>" name="searchForm" id="searchForm" method="post" class="boardSearch">
			...
			<div class="search_box" style="padding:10px;">
				<div class="board-search">
					<label  for="searchReceipt1" >자료구분 :</label>
					<select name="searchDataType" id="searchDataType" class="select" style="height: 24px; margin-right: 15px;">
					...
					<!-- 검색종류선택 -->
					<label  for="searchReceipt2" >검색구분 :</label>
					...
					<!-- 키워드 -->
					<input type="text" class="inp_txt" name="searchWord" id="searchWord" value="<%=util.getStr(paramMap.get("searchWord"))%>" style="width:66%;" />
				...
				<tr>
					...
					<td class="subject">
						<a href="#" onclick="goSubmit('noticeView','<%=util.getStr(String.valueOf(rs.get("ARTICLE_ID")))%>','<%=util.getStr(rs.get("BOARD_ID"))%>')">
							...
							<%if(util.getStr(rs.get("SUBJECT")).length()>25){%><%=util.getStr(rs.get("SUBJECT")).substring(0, 25)+" · · · "%><%}else{%><%=util.getStr(rs.get("SUBJECT"))%><%}%>
						</a>
					</td>
					<td>
