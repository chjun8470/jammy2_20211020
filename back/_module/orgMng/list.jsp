...
<form id="searchForm" name="searchForm" method="post" action="/sys/sys_org_manage.do">
	...
	<div class="basic_grid">
		<div class="search_box" style="padding:10px">
			...
			<label for="searchType1">구분 :</label>
			<select name="orgType" id="orgType" class="search_box_dn">
				<option value="" <%if("".equals(util.getStr(paramMap.get("orgType")))) {%>selected="selected"<%} %>>전체</option>
				<option value="1" <%if("1".equals(util.getStr(paramMap.get("orgType")))) {%>selected="selected"<%} %>>기업</option>
				<option value="2" <%if("2".equals(util.getStr(paramMap.get("orgType")))) {%>selected="selected"<%} %>>기관/센터</option>
			</select>

			<label for="searchType2" style="padding-left:15px;">상태 :</label>
			<select name="applyCd" id="applyCd" class="search_box_dn">
				<option value="" <%if("".equals(util.getStr(paramMap.get("applyCd")))) {%>selected="selected"<%} %>>전체</option>
				<option value="10" <%if("10".equals(util.getStr(paramMap.get("applyCd")))) {%>selected="selected"<%} %>>신청</option>
				<option value="20" <%if("20".equals(util.getStr(paramMap.get("applyCd")))) {%>selected="selected"<%} %>>승인</option>
				<option value="30" <%if("30".equals(util.getStr(paramMap.get("applyCd")))) {%>selected="selected"<%} %>>반려</option>
			</select>

			<label for="searchType3" style="padding-left:20px;">기업/기관명 :</label>
		...
		<div class="btn_box">
			<div class = "btn_right_box">
				<input type="button" class="btn_inp_b_01" value="기업/기관 추가" onclick="pageWriteGo()">
			</div>
		</div>
