		...
		<div class="left_box w75">
				...
				<div class="fr">
					...
					<table class="skin_basic_write">
						...
						<tbody>
							...
							<tr  id="authCdTr" <%=!util.getStr(dataMap.get("ORG_TYPE")).equals("2") ? "style='display:none'":"" %>>
								<th scope="col"><label for="authCd1">관리권한</label></th>
								<td colspan="3">
									<label><input type="radio" id="authCd1" name="authCd" value="1" <%=util.getStr(dataMap.get("AUTH_CD")).equals("1")? "checked='checked'": ""%>/>일반</label>
									<label><input type="radio" id="authCd2" name="authCd" value="2" <%=util.getStr(dataMap.get("AUTH_CD")).equals("2")? "checked='checked'": ""%>/>관리자</label>
								</td>
							</tr>
							<tr id="authOrgSiteTr"  <%=!util.getStr(dataMap.get("AUTH_CD")).equals("2")? "style='display:none'": ""%>>
								<th scope="col"><label for="authOrgSite1">관리기관</label></th>
								<td colspan="3">
									연구장비정보망 : 
									<label><input type="radio" id="authOrgSite1" name="authOrgSite1" value="Y" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_1")).equals("Y")? "checked='checked'": ""%>/>Y</label>
									<label><input type="radio" id="authOrgSite1" name="authOrgSite1" value="N" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_1")).equals("N")? "checked='checked'": ""%>/>N</label>
									<br/>
									지식재산센터 : 
									<label><input type="radio" id="authOrgSite2" name="authOrgSite2" value="Y" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_2")).equals("Y")? "checked='checked'": ""%>/>Y</label>
									<label><input type="radio" id="authOrgSite2" name="authOrgSite2" value="N" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_2")).equals("N")? "checked='checked'": ""%>/>N</label>
									<br/>
									과학기술진흥센터 : 
									<label><input type="radio" id="authOrgSite3" name="authOrgSite3" value="Y" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_3")).equals("Y")? "checked='checked'": ""%>/>Y</label>
									<label><input type="radio" id="authOrgSite3" name="authOrgSite3" value="N" <%=util.getStr(dataMap.get("AUTH_ORG_SITE_3")).equals("N")? "checked='checked'": ""%>/>N</label>
								</td>
							</tr>
							<tr>
								<th scope="col"><label for="orgGrpNm">기관명</label></th>
								<td colspan="3"><input type="text" id="orgGrpNm" name="orgGrpNm" maxlength="10" value="<%=util.getStr(dataMap.get("ORG_GRP_NM"))%>"></td>
							</tr>
							<tr>
								<th scope="col"><label for="areaCd1">소속지역</label></th>
								<td colspan="3">
									<label>
										<input type="radio" id="areaCd1" name="areaCd" value="1" <%=util.getStr(dataMap.get("AREA_CD")).equals("1")? "checked='checked'": ""%>/>전라남도 기관
									</label>
									<label>
										<input type="radio" id="areaCd1" name="areaCd" value="2" <%=util.getStr(dataMap.get("AREA_CD")).equals("2")? "checked='checked'": ""%>/>지역 외 기관
									</label>
								</td>
							</tr>
							<tr>
								<th scope="col"><label for="ornerNm">대표자명</label></th>
								<td colspan="3"><input type="text" id="ornerNm" name="ornerNm" maxlength="10" value="<%=util.getStr(dataMap.get("ORNER_NM"))%>"></td>
								
							</tr>
							<tr>
								<th scope="col"><label for="corpNum1">사업자등록번호</label></th>
								<td colspan="3"><input type="text" id="corpNum1" name="corpNum1" maxlength="20" value="<%=util.getStr(dataMap.get("CORP_NUM_1"))%>"></td>
							</tr>
							<tr>
								<th scope="col"><label for="corpNum2">법인등록번호</label></th>
								<td colspan="3"><input type="text" id="corpNum2" name="corpNum2" maxlength="20" value="<%=util.getStr(dataMap.get("CORP_NUM_2"))%>"></td>
							</tr>
						<tr>
							<th scope="col"><label for="corpPhone">대표전화번호</label></th>
							<td ><input type="text" id="corpPhone" name="corpPhone" maxlength="20" value="<%=util.getStr(dataMap.get("CORP_PHONE"))%>"></td>
							<th scope="col"><label for="corpFax">대표FAX번호</label></th>
							<td ><input type="text" id="corpFax" name="corpFax" maxlength="20" value="<%=util.getStr(dataMap.get("CORP_FAX"))%>"></td>
						</tr>
						<tr>
							<th scope="col"><label for="homepage">홈페이지</label></th>
							<td colspan="3"><input type="text" id="homepage" name="homepage" maxlength="50" value="<%=util.getStr(dataMap.get("HOMEPAGE"))%>"></td>
						</tr>
						<tr>
							<th scope="col"><label for="addr1">기관주소</label></th>
							<td colspan="3">
								<input type="text" id="zipcode" name="zipcode" maxlength="10" readonly="readonly" value="<%=util.getStr(dataMap.get("ZIPCODE"))%>">
								<input type="text" id="addr1" name="addr1" maxlength="50" readonly="readonly" value="<%=util.getStr(dataMap.get("ADDR1"))%>">
						...
						<tr>
							...
							<td colspan="3"><input type="text" id="addr2" name="addr2" maxlength="50" value="<%=util.getStr(dataMap.get("ADDR2"))%>"></td>
						</tr>
						<tr  id="userIdxTr" <%=!util.getStr(dataMap.get("ORG_TYPE")).equals("2") ? "style='display:none'":"" %>>
							<th scope="col"><label for="name">기관관리자</label></th>
							<td colspan="3">
								<input type="hidden" id="num1" name="userIdx" maxlength="15" value="<%=util.getStr(dataMap.get("USER_IDX"))%>">
								<input type="text" id="name" name="name" maxlength="15" value="<%=util.getStr(dataMap.get("PSN_NM"))%>">
						...
						<tr>
							...
							<td colspan="3">
							X : <input type="text" id="mapX" name="mapX" maxlength="20" value="<%=util.getStr(dataMap.get("MAP_X"))%>">, 
							Y : <input type="text" id="mapY" name="mapY" maxlength="20" value="<%=util.getStr(dataMap.get("MAP_Y"))%>">
							<div id="map" style="width:100%;height:300px;margin-top:10px;"></div>
							</td>
						</tr>
						<tr>
							<th scope="col"><label for="orgNote1">기관분류</label></th>
							<td colspan="3"><textarea id="orgNote1" name="orgNote1" cols="50" rows="4" maxlength="500"><%=util.getStr(dataMap.get("ORG_NOTE_1"))%></textarea></td>
						</tr>
						<tr>
							<th scope="col"><label for="orgNote2">주요제품</label></th>
							<td colspan="3"><textarea id="orgNote2" name="orgNote2" cols="50" rows="4" maxlength="500"><%=util.getStr(dataMap.get("ORG_NOTE_2"))%></textarea></td>
						</tr>
						<tr>
							<th scope="col"><label for="orgNote3">보유기술현황</label></th>
							<td colspan="3"><textarea id="orgNote3" name="orgNote3" cols="50" rows="4" maxlength="500"><%=util.getStr(dataMap.get("ORG_NOTE_3"))%></textarea></td>

						</tr>
						<tr>
							<th scope="col"><label for="orgNote4">연구분야</label></th>
							<td colspan="3"><textarea id="orgNote4" name="orgNote4" cols="50" rows="4" maxlength="500"><%=util.getStr(dataMap.get("ORG_NOTE_4"))%></textarea></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<ul class="file" style="line-height: 22px;">
								<li class="text">
									...
									<img class='ci_img' src="<%=util.getStr(dataMap.get("CI_FILE_SRC"))%>" alt='CI이미지' style='width:210px;height:38px'/>
								</li>
							</ul>
			...
			<div class="btn_box">
				...
				<div class="btn_right_box">
					...
					<button class="btn_inp_w_01" type="button" onclick="pageViewGo()">뒤로</button>
				</div>
			</div>
		</div>
</form>
