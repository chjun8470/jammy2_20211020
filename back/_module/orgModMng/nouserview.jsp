	...
	for(HashMap rs:dataList) {
		...
		%>
			<div class="menuRows_1"  onmouseover="menuOver(1,this)" onmouseout="menuOut(1,this)" style="position:relative;">
				<input type="button" value="&lt;" class="btn_y_01" onclick="authOrgAdd('<%=util.getStr(rs.get("USER_IDX"))%>')" style="position:absolute;top:20px;right:10px;width:30px;min-width:30px;z-index:8;"/>
				&nbsp;
				<label style="display:inline-block;width:100%;padding-left:5px;" onclick="subOrgAuthShow('user','<%=util.getStr(rs.get("USER_IDX"))%>','<%=util.getStr(rs.get("PSN_NM"))%>','<%=util.getStr(rs.get("USER_ID"))%>',
