	...
	<div class="bbs-top">
			...
			<div class="search_box" style="padding:10px">
				...
				<select name="searchType" id="searchType" class="select" style="height: 24px">
					<option value="SUB_NAME" <%if("SUB_NAME".equals(util.getStr(paramMap.get("searchType")))) {%> selected="selected" <%}%>>부서/센터 명</option>
				</select>
		...
		<table class="skin_basic_list">
			...
			for(HashMap rs:dataList) {%>
				<tr>
					...
					<td class="name">
						<button id = "bu" class="btn_inp_w_01"  onclick="clickToBtn('<%=util.getStr(rs.get("ORG_GRP_SUB_CD"))%>','<%=util.getStr(rs.get("ORG_GRP_SUB_NM"))%>')">선택</button>
					</td>
				</tr>
			<%
			cont--;
...
<script type="text/javascript">
	...
	function clickToBtn(idx,nm){
			$(opener.document).find("input[name=OwnDept]").val(idx);
			$(opener.document).find("input[name=OwnDeptNm]").val(nm);
			window.close();
	}
