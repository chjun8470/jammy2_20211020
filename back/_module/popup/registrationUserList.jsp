		...
		<div class="basic_grid">
			<div class="search_box" style="padding:10px">
				<select id="searchType" name="searchType">
					<option value="ID" <%if("ID".equals(searchType)) {%>selected="selected"<%} %>>회원ID</option>
					<option value="NAME" <%if("NAME".equals(searchType)) {%>selected="selected"<%} %>>회원명</option>
		...
		<table class="skin_basic_list">
			...
			for(HashMap rs:dataList) {%>
				<tr>
					...
					<td>
						<button id = "bu" class="btn_inp_w_01"  onclick="clickToBtn('<%=util.getStr(rs.get("ORG_GRP_NM"))%>','<%=util.getStr(rs.get("DEPT_NM"))%>','<%=util.getStr(rs.get("PSN_NM"))%>','<%=util.getStr(rs.get("POSITION_NM"))%>','<%=util.getStr(rs.get("TEL_NO"))%>','<%=util.getStr(rs.get("EMAIL"))%>','<%=util.getStr(rs.get("USER_IDX"))%>')">선택</button>
					</td>
				</tr>
			<%
			cont--;
