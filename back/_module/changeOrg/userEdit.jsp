...
<%--                 			<td colspan="3"><%=util.getStr(dataMap.get("USER_APP_STATE"))%></td> --%>
                			...
                			<select id="userAppState" name="userAppState" onchange="changeState(this.value)">
                				<option value="10" <%=util.getStr(dataMap.get("CHANGE_ORG")).equals("요청")? "selected='selected'" : "" %>> 요청 </option>
                				<option value="20" <%=util.getStr(dataMap.get("CHANGE_ORG")).equals("승인")? "selected='selected'" : "" %>> 승인 </option>
                				<option value="30" <%=util.getStr(dataMap.get("CHANGE_ORG")).equals("반려")? "selected='selected'" : "" %>> 반려 </option>
                			</select>
                			</td>
                		</tr>
                		<tr id="trAcceptNo" <%if(!util.getStr(dataMap.get("CHANGE_ORG")).equals("반려")){ %> style="display:none" <% }%>>
                			<th scope="col"><label for="">반려사유</label></th>
                			<td colspan="3"><textarea id="acceptNo" name="acceptNo" style="width:500px; height:100px"><%=util.getStr(dataMap.get("CHANGE_ORG_NO"))%></textarea></td>
                		</tr>
                    	<tr>
