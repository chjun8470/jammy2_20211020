...
<% 
	...
	String boardComIdx = "00067";//코드를 입력하세요
	out.println(listMode);
%>

<jsp:include page="/web/comBoard.do" flush="true">
	<jsp:param name="mode" value="<%=mode%>"/>
	<jsp:param name="siteId" value="smbrnd"/>
...
<script>
    $(function() {
        ...
        $(".business_tab .tab li").on("click", function() {
            var $this = $(this);
            var tab_idx = $this.index();
            var $tab_con = $(".tab_content");
            
            $(".business_tab .tab li").filter(".on").removeClass("on");
    
            $this.addClass("on");
    
            $tab_con.removeClass("active");
            $tab_con.eq(tab_idx).addClass("active");
            
            
            $("#searchForm"+(tab_idx+1))[0].reset();
            $("#searchForm"+(tab_idx+1))[0].submit();
        });
