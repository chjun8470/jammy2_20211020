...
public void jspInit() {
	...
	sql_getJnspDay.append("			AND A.SITE_ID = 'jipbank' ");
	...
	sql_getJnspDayTb.append("			AND A.SITE_ID = 'jipbank' ");
	...
	sql_getJnspDayTt.append("			AND A.SITE_ID = 'jipbank' ");
...
class ReportHandler{
	...
	private void getScriptFromyyyyy(boolean bIsFirst, boolean bIsPrint) throws Exception{
		...
		if(bIsFirst){   //밴드 최초 호출 시에만 실행되는 코드
			//DataSet 객체 생성
			//ResultSet getJnspDayTt 객체의 생성
			if(getJnspDayTt==null){
				getJnspDayTt = stat_getJnspDayTt.executeQuery();
			}else{
				getJnspDayTt.beforeFirst();
			}

			//데이터 fetch
			if(getJnspDayTt.next()){
				setFieldVariableFromgetJnspDayTt();	//ResultSet getJnspDayTt에서 필드 값 설정
			}else{
				setFieldInitFromgetJnspDayTt();	//ResultSet getJnspDayTt의 필드 값 초기화
			}
	...
	private void getScriptFromTbList(boolean bIsPrint) throws Exception{
		...
		//DataSet 객체 생성
		//ResultSet getJnspDayTb 객체의 생성
		if(getJnspDayTb==null){
			getJnspDayTb = stat_getJnspDayTb.executeQuery();
		}else{
			getJnspDayTb.beforeFirst();
		}
		...
		//--while Loop Start--##
		while(getJnspDayTb.next()){   //자동으로 생성되는 반복 밴드의 루프문
			//데이터 fetch
			setFieldVariableFromgetJnspDayTb();	//RecordSet getJnspDayTb에서 필드 값 설정

			if(bIsFirst){   //밴드 호출 후 루프 내에서 한번 만 실행되는 코드
			}   //end if(bIsFirst)

			//데이터 변수 할당
			sheet10_B21 = getJnspDayTb_AXIS;
			sheet10_N21 = getJnspDayTb_CNT;
	...
	private void getChartDataAI_CHART_OBJ32( ) throws Exception{
		//DataSet 객체 생성
		//ResultSet getJnspDay 객체의 생성
		if(getJnspDay==null){
			getJnspDay = stat_getJnspDay.executeQuery();
		}else{
			getJnspDay.beforeFirst();
		}
		if(!getJnspDay.isBeforeFirst()){		//생성된 ResultSet에 레코드가 없으면
			return;
		}
		...
		while(getJnspDay.next()){   //데이터 루프문
			//데이터 fetch
			setFieldVariableFromgetJnspDay();   //RecordSet getJnspDay에서 필드 값 설정

			//X축 레이블 필드 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + getJnspDay_AXIS + "\t";

			//데이터 설정
			AI_CHART_OBJ32=AI_CHART_OBJ32 + getJnspDay_CNT + "\r\n";
		}   //while end
	}
