...
<div class="MoreView">
	...
	<dl class="MoreView04">
		...
		<% } else if(util.getStr(dataMap.get("USE_SCOPE_MEAN")).equals("분석ㆍ시험의뢰")) { %>
...
<div class="MoreTxt">
	...
	<div class="MoreTop">
		<div class="MoreTopNo">고정자산관리번호 : <%=util.getStr(dataMap.get("FIXED_ASET_NO"))%>
			...
			<%if(util.getStr(dataMap.get("EQU_SATISFACTION")).equals("1.0")) { %> <img src="/img/board/star1.png" alt="1점" /> 1점 <% } %>
			<%if(util.getStr(dataMap.get("EQU_SATISFACTION")).equals("2.0")) { %> <img src="/img/board/star2.png" alt="2점" /> 2점 <% } %>
			<%if(util.getStr(dataMap.get("EQU_SATISFACTION")).equals("3.0")) { %> <img src="/img/board/star3.png" alt="3점" /> 3점 <% } %>
			<%if(util.getStr(dataMap.get("EQU_SATISFACTION")).equals("4.0")) { %> <img src="/img/board/star4.png" alt="4점" /> 4점 <% } %>
			<%if(util.getStr(dataMap.get("EQU_SATISFACTION")).equals("5.0")) { %> <img src="/img/board/star5.png" alt="5점" /> 5점 <% } %>
	...
	<dl>
		...
		<dt>보유기관</dt>
		<dd><%=util.getStr(dataMap.get("OWN_ORG_NM"))%></dd>
...
<dt>이용방법</dt>
<dd>&nbsp;<%=util.getStr(dataMap.get("USE_SCOPE_MEAN"))%></dd>
...
<%if(!util.getStr(dataMap.get("MAP_X")).equals(null) && !util.getStr(dataMap.get("MAP_X")).equals("")) { %>
	...
	<%-- X : <input type="text" name = "mapX" id = "mapX" maxlength="20" value="<%=dataMap.get("MAP_X")%>"/>
	< br/> Y : <input type="text" name = "mapY" id = "mapY" maxlength="20" value="<%=dataMap.get("MAP_Y")%>"/> --%>
	<div id="map" style="width:710px;height:250px;margin-top:10px;"></div>
	<% } %>
	<% } else { %>
	&nbsp;&nbsp;&nbsp;설치장소 정보가 없습니다.
<% } %>
...
<dt>연락처</dt>
<dd>&nbsp;- 장비담당자 : <%=util.getStr(dataMap.get("REF_EQU_TEL"))%><br />
	&nbsp;- 예약담당자 : <%=util.getStr(dataMap.get("REF_RSV_TEL"))%></dd>
</dl>
</div>


<%-- <%if(!"".equals(util.getStr(loginVO.getUserIdx()))) { %>
		<p class="down_publist"><a href="#" onclick="pageFavGo('<%=util.getStr(paramMap.get("rndEquIdx"))%>')">

		<%if("1".equals(util.getStr(paramMap.get("FAV_COUNT")))) { %>
		<img alt="장바구니" src="/img/back/comm/fav_btn_2.png"></a></p>
		<%} else { %>
		<img alt="장바구니" src="/img/back/comm/fav_btn_1.png"></a></p>
		<%} %>
	<%} %> --%>
...
<%} %>
	...
	var mapX = <%=dataMap.get("MAP_X")%>;
	var mapY = <%=dataMap.get("MAP_Y")%>;
