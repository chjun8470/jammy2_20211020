...
<div class="skin_list">
	<div>
                    ...
                    <div class="list_top_1 margin_left_20px">
                     	<div class="b_srchBox_1 ">
							...
							<div class="b_srchBox_1_du">
								행사참가비 :
								&nbsp;&nbsp;<input type="radio" name="searchPay" value="" <%if("".equals(util.getStr(paramMap.get("searchPay")))) {%> checked="checked" title="행사참가비 전체" <%}%>/> 전체
								&nbsp;&nbsp;<input type="radio" name="searchPay" value="N" <%if("N".equals(util.getStr(paramMap.get("searchPay")))) {%> checked="checked" title="행사참가비 무료" <%}%>/> 무료
								&nbsp;&nbsp;<input type="radio" name="searchPay" value="Y" <%if("Y".equals(util.getStr(paramMap.get("searchPay")))) {%> checked="checked" title="행사참가비 유료" <%}%>/> 유료
							</div>
		...
		<div class="list_top">
			...
			<div class="publist_top_wrap02">
				<div class="b_btn_listBox" style="margin-right:10px;">
					<button type="button" class="b_btn_list" onclick="goSubmit('list','list')"><img src="/img/board/btn_list.png" alt="게시판형"/></button>
					<button type="button" class="b_btn_blog" onclick="goSubmit('list','imageList')"><img src="/img/board/btn_blog_ovr.png" alt="블로그형"/></button>
		...
		<div class="DateList03" id="cont_block">
				...
				%>
				<dl class="dat_list">
				<dt style="cursor:pointer;" onclick="goSubmit('view','imageList','','','<%=util.getStr(rs.get("EVENT_IDX"))%>')">
					<span class="noL"><%=cont %></span>
					<span class="titL"><%if(util.getStr(rs.get("EVENT_NM")).length()>30){%><%=util.getStr(rs.get("EVENT_NM")).substring(0, 30)+" · · · "%><%}else{%><%=util.getStr(rs.get("EVENT_NM"))%><%}%></span>
				</dt>
