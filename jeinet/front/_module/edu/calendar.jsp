...
%>
	...
	<div class="list_top">
		  ...
		  <div class="public_top_wrap">
		  	<div class="b_btn_listBox">
				...
				<button type="button" class="b_btn_blog" onclick="goSubmit('list','calendar',<%=nowYear%>,<%=nowMonth%>)""><img src="/img/board/btn_calendar_ovr.png" alt="달력형"/></button>
			</div>
	...
	<div class="calendar_box" id="cont_block">
		...
		<div class="calendar_in" style="float:left;">
			...
			<!-- 년/월 선택 -->
			<p class="cal-ym01">
				<span class="prev"><a href="sub.do?m=<%=m%>&mode=<%=mode %>&listMode=calendar&year=<%=ppreYear %>&month=<%=preMonth %>" onclick="checkCalendar(1)">◀</a></span>
				<span><%=year%>년</span>
				<span class="next"><a href="sub.do?m=<%=m%>&mode=<%=mode %>&listMode=calendar&year=<%=nnextYear %>&month=<%=nextMonth %>" onclick="checkCalendar(2)">▶</a></span>
				<span class="prev"><a href="sub.do?m=<%=m%>&mode=<%=mode %>&listMode=calendar&year=<%=preYear %>&month=<%=ppreMonth %>" onclick="checkCalendar(3)">◀</a></span>
				<span><%=month%>월</span>
				<span class="next"><a href="sub.do?m=<%=m%>&mode=<%=mode %>&listMode=calendar&year=<%=nextYear %>&month=<%=nnextMonth %>" onclick="checkCalendar(4)">▶</a></span>
			</p>
