	...
	<form action="./sub.do" name="searchForm" id="searchForm" method="post" class="boardSearch">
		...
		<div class="publist_top_wrap02">
			<div class="b_btn_listBox" style="margin-right:10px;">
				<button type="button" class="b_btn_list" onclick="goButton('list','list')"><img src="/img/board/btn_list_ovr.png" alt="게시판형"/></button>
				<button type="button" class="b_btn_blog" onclick="goButton('list','imageList')"><img src="/img/board/btn_blog.png" alt="블로그형"/></button>
				<button type="button" class="b_btn_blog" onclick="goButton('list','calendar',<%=nowYear%>,<%=nowMonth%>)"><img src="/img/board/btn_calendar.png" alt="달력형"/></button>
			</div>
		...
		<div id="cont_block">
			...
			<div class="basic_listWrap">
				...
				<tr>
						...
						<th scope="col" class="r_line_none2">접수기간</th>
						<th scope="col" class="r_line_none2">교육기간</th>
						<th scope="col" class="number">참여인원</th>
						<th scope="col" class="r_line_none2">교육상태</th>
				</tr>
				</thead>
				<tbody>
					<%
					...
					%>
						<tr style="cursor:pointer;" onmouseover="this.style.background='#f3f3f3'" onmouseout="this.style.background='white'" onclick="goSubmit('view','<%=util.getStr(rs.get("EDU_ID"))%>')">
							<td class="number"><%=cont%></td>
							<td class="r_line_none2"><%=util.getStr(rs.get("EDU_NM"))%><%=util.getStr(rs.get("EDU_CUR"))%></td>
							<td class="r_line_none2"><%=util.getStr(rs.get("EDU_ORD_ST_DT"))%> ~ <%=util.getStr(rs.get("EDU_ORD_ED_DT"))%></td>
							<td class="r_line_none2"><%=util.getStr(rs.get("EDU_ST_DT"))%> ~ <%=util.getStr(rs.get("EDU_ED_DT"))%></td>
							<td class="number"><%=util.getIntStr(rs.get("EDU_ORD_PEOPLE"))%>/<%=util.getIntStr(rs.get("EDU_PEOPLE"))%></td>
							<td class="r_line_none2 line_cnci_lft">
