						...
						<tbody>
							...
							<tr id="companyType" <%if(!util.getStr(dataMap.get("ORG_TYPE")).equals("1")) { %> style="display: none;" <% } %>>
								...
								<%for(HashMap rs : companyType) { %>
									<input type="radio" name="companyType" value="<%=rs.get("CODE_CD") %>" <%if(util.getStr(dataMap.get("COMPANY_TYPE_CD")).equals(util.getStr(rs.get("CODE_CD")))) { %> checked="checked" <% } %> /><%=rs.get("CODE_NM") %>
								<% } %>
								</td>
							</tr>
							<tr id="agencyType" <%if(!util.getStr(dataMap.get("ORG_TYPE")).equals("2")) { %> style="display: none;" <% } %>>
								...
								<%for(HashMap rs : agencyType) { %>
									<input type="radio" name="agencyType" value="<%=rs.get("CODE_CD") %>" <%if(util.getStr(dataMap.get("AGENCY_TYPE")).equals(util.getStr(rs.get("CODE_CD")))) { %> checked="checked" <% } %> /><%=rs.get("CODE_NM") %>
								<% } %>
								</td>
							</tr>
							<tr>
							...
							<tr  id="authCdTr" <%=!util.getStr(dataMap.get("ORG_TYPE")).equals("2") ? "style='display:none'":"" %>>
								<th scope="col"><label for="authCd1"><span class="required">*</span>관리권한</label></th>
								<td colspan="3">
							...
							<tr  id="userIdxTr" <%=!util.getStr(dataMap.get("ORG_TYPE")).equals("2") ? "style='display:none'":"" %>>
								...
								<td colspan="3">
									...
									<input type="text" class="inp_text" id="name" name="name"  maxlength="15" value="<%=util.getStr(dataMap.get("PSN_NM"))%>" style="width:17%;"/>
