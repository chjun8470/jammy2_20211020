...
<div class="clear_wrap">
	<h2>장비이용 예약 신청서</h2>
	                	...
	                	<tbody>
							<tr>
	                			...
	                			<th scope="col">기관접수번호</th>
	                				<%=util.getStr(dataMap.get("ORG_RECEIPT_NO"))%>
	                			<td scope="col" >
							...
							<tr>
	                			...
	                			<td colspan="3">
	                				<%if(util.getStr(dataMap.get("ORDER_STATE")).equals("1")) { %> <img alt="승인요청" src="/img/back/comm/order_state_img_1.png"> <button onclick="printPop('<%=dataMap.get("RND_EQU_IDX")%>','<%=dataMap.get("USER_IDX")%>','<%=util.getStr(paramMap.get("equOrderIdx"))%>')">인쇄하기</button> <% } %>
                	...
                	<table id="step1"  class="skin_basic_write">
	                	...
	                	<tbody>
	                		...
	                		<tr>
	                			...
	                			<td scope="col">
		                			<%=util.getStr(dataMap.get("SAMPLE_NM"))%>(<%=util.getStr(dataMap.get("SAMPLE_NO"))%>)
	                			</td>
		...
		<table id="step1"  class="skin_basic_write">
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="3">
						...
						<%=util.getStr(diaryDataMap.get("START_TIME")) %>시  ~
						<%=util.getStr(String.valueOf(diaryDataMap.get("END_DT"))) %>
						<%=util.getStr(diaryDataMap.get("END_TIME")) %>시
					</td>
				</tr>
				<tr style="display: none;">
              	...
              	<div class="btn_left_box" >
              		...
              		<% if(util.loginCheck()) {%>
		            	...
		            	<% } else if(util.getStr(dataMap.get("ORDER_STATE")).equals("2")) { %> <!-- 이용승인상태일경우 -->
		            		...
		            		<% } %> --%>
		            			<button class="btn_inp_b_01" type="button" onclick="resveConsentGo('5')">승인 취소</button>
			...
			반려 사유 : <textarea rows="3" cols="50" id="rejectTxt" name="rejectTxt"></textarea>

			<button class="btn_inp_b_01" type="button" onclick="resveConsentGo('3')">반려확인</button> </div>
	...
	<%if(util.getStr(dataMap.get("ORDER_STATE")).equals("4") && util.getStr(dataMap.get("DIARY_YN")).equals("")) { %>
		...
		<div class="skin_bbs_write" id="operDiaryWrite" style="display:none;">
		<h3>운영일지 작성</h3>
...
<form id = "fview" name = "fview" method="post" action="/sys/sys_eq_reservation.do">
	...
	<input type="hidden" id="orderState" 	name="orderState"	value=""/>
